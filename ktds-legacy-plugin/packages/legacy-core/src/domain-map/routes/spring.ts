...
/** HTTP 동사 어노테이션 -> 메서드. */
const VERB_BY_MAPPING: Record<string, RouteMethod> = {
  GetMapping: 'GET',
  PostMapping: 'POST',
  PutMapping: 'PUT',
...
function requestMethodToken(text: string): RouteMethod | null {
  const m = text.trim().replace(/^RequestMethod\./, '')
  const known: RouteMethod[] = ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'HEAD', 'OPTIONS']
  return (known as string[]).includes(m) ? (m as RouteMethod) : null
}
...
/**
 * 어노테이션 인자 표현식 1개를 경로 문자열로 해소한다.
 * 해소 성공 시 { value, note? }, 실패 시 { unresolved: ref, note }.
 */
interface Resolved {
  value: string | null
...
function resolveExpr(node: Node, ctx: SpringContext): Resolved {
  switch (node.type) {
    case 'string_literal':
    ...
    case 'identifier': {
      ...
      if (v !== undefined) return { value: v, note: `constant:${ref}` }
...
function extractMappingArgs(
  ...
  if (pairs.length > 0) {
    ...
    for (const pair of pairs) {
      ...
      const valueNode = pair.namedChildren.filter((c): c is Node => c !== null)[1]
  ...
  for (const child of argList.namedChildren) {
    if (child) collectPathExprs(child, ctx, paths)
  }
  return { paths, methods }
}

/** 경로 표현식(단일/배열)을 후보로 누적. */
function collectPathExprs(node: Node, ctx: SpringContext, out: PathCandidate[]): void {
  if (node.type === 'block_comment' || node.type === 'line_comment') return
  if (node.type === 'element_value_array_initializer') {
    for (const child of node.namedChildren) {
      if (child) collectPathExprs(child, ctx, out)
    }
    return
  }
...
function collectMethods(node: Node, out: RouteMethod[]): void {
  if (node.type === 'element_value_array_initializer') {
    for (const child of node.namedChildren) {
      if (child) collectMethods(child, out)
    }
    return
  }
...
function methodName(method: Node): string | null {
  ...
  if (fpIdx > 0) {
    ...
    if (before.type === 'identifier') return before.text
  }
  // 폴백: 첫 identifier.
...
function methodAnnotations(method: Node): Node[] {
...
/** 클래스의 어노테이션 노드들. */
function classAnnotations(cls: Node): Node[] {
...
/** 클래스 이름. */
function className(cls: Node): string | null {
 ...
 */
export function extractSpringRoutes(root: Node, filePath: string, ctx: SpringContext): RouteEntry[] {
  ...
  for (const cls of classes) {
    ...
    const annotNames = annots.map(annotationName).filter((n): n is string => n !== null)

    // composed stereotype 까지 펼친 클래스 어노테이션 집합.
    const composedClassNotes: string[] = []
    ...
    for (const method of methods) {
      ...
      for (let i = 0; i < mAnnots.length; i++) {
        ...
        let verb: RouteMethod | undefined
        ...
        // 메서드 집합 결정.
        let effectiveMethods: RouteMethod[]
        if (name in VERB_BY_MAPPING) {
          effectiveMethods = [verb as RouteMethod]
...
function returnsResponseEntity(method: Node): boolean {
  return /(^|[^A-Za-z])ResponseEntity\b/.test(returnTypeText(method))
}

/** 노트를 중복 제거 + 정렬(결정론). */
function dedupSort(notes: string[]): string[] {
...
function findClassDeclarations(root: Node): Node[] {
  ...
  while (stack.length > 0) {
    ...
    for (const child of node.namedChildren) {
      if (!child) continue
      if (child.type === 'class_declaration') out.push(child)
      stack.push(child)
    }
  }
  return out
 ...
 */
export function collectConstants(root: Node, into: Map<string, string>): void {
  const classes = findClassDeclarations(root)
  for (const cls of classes) {
    const clsName = className(cls)
    const body = childrenOfType(cls, 'class_body')[0]
    ...
    for (const field of childrenOfType(body, 'field_declaration')) {
      ...
      if (!/\bstatic\b/.test(modText) || !/\bfinal\b/.test(modText)) continue
      for (const decl of childrenOfType(field, 'variable_declarator')) {
        ...
        if (!nameId || !lit) continue
        const value = stringLiteralValue(lit)
        if (clsName) into.set(`${clsName}.${nameId.text}`, value)
 ...
 */
export function collectComposedAnnotations(
  root: Node,
  composedVerb: Map<string, RouteMethod | undefined>,
  composedStereotype: Set<string>,
): void {
  ...
  while (stack.length > 0) {
    ...
    for (const child of node.namedChildren) {
      ...
      if (child.type === 'annotation_type_declaration') {
        ...
        if (id) classifyComposed(child, id.text, composedVerb, composedStereotype)
      }
      stack.push(child)
    }
  }
      ...
      if (argList) {
        for (const pair of childrenOfType(argList, 'element_value_pair')) {
          if (childrenOfType(pair, 'identifier')[0]?.text === 'method') {
            const valueNode = pair.namedChildren.filter((c): c is Node => c !== null)[1]
            if (valueNode) collectMethods(valueNode, methods)
          }
        }
      }
