...
export const COMPLEXITY_ALIASES = {
    low: "simple",
    easy: "simple",
    medium: "moderate",
...
export function sanitizeGraph(data) {
    ...
    if (data.tour === null || data.tour === undefined)
        result.tour = [];
    if (data.layers === null || data.layers === undefined)
        result.layers = [];
    // Sanitize nodes
    if (Array.isArray(data.nodes)) {
        result.nodes = data.nodes.map((node) => {
            ...
            if (typeof n.complexity === "string")
                n.complexity = n.complexity.toLowerCase();
            return n;
        });
    }
    // Sanitize edges
    ...
    if (Array.isArray(result.tour)) {
        result.tour = result.tour.map((step) => {
...
export function autoFixGraph(data) {
    const issues = [];
    const result = { ...data };
    if (Array.isArray(data.nodes)) {
        result.nodes = data.nodes.map((node, i) => {
            if (typeof node !== "object" || node === null)
                return node;
            const n = { ...node };
            const name = n.name || n.id || `index ${i}`;
    ...
    if (Array.isArray(data.edges)) {
        result.edges = data.edges.map((edge, i) => {
            if (typeof edge !== "object" || edge === null)
                return edge;
            const e = { ...edge };
            // Missing type
            ...
            if (!e.direction || typeof e.direction !== "string") {
                ...
                e.direction = DIRECTION_ALIASES[e.direction];
                issues.push({
                    level: "auto-corrected",
                    category: "alias",
...
function buildInvalidCollectionIssue(name) {
    return {
        level: "fatal",
...
export function validateGraph(data) {
    ...
    const normalized = normalizeGraph(sanitized);
    ...
    const requiredCollections = ["nodes", "edges", "layers", "tour"];
