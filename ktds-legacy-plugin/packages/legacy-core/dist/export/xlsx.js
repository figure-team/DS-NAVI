...
function sheetXml(sheet) {
    ...
    for (let c = 0; c < colCount; c++) {
        ...
        for (const row of sheet.rows) {
            ...
            let len = 0;
            for (const ch of v)
                len += ch.charCodeAt(0) > 0x2e7f ? 1.7 : 1;
    ...
    const autoFilter = hasHeader && colCount > 0 && sheet.rows.length > 1
 ...
 */
export function sanitizeSheetNames(names) {
    const used = new Set();
...
export function buildXlsxWorkbook(sheets) {
    if (sheets.length === 0)
        throw new Error('xlsx: 시트가 최소 1개 필요합니다');
