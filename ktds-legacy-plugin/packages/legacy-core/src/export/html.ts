...
function evidenceSpan(evidence: Evidence[]): string {
...
function renderClaimLi(claim: Claim): string {
...
function renderTable(table: Table): string[] {
  ...
  for (const col of header) lines.push(`<th>${escapeHtml(col)}</th>`)
  lines.push('</tr>', '</thead>', '<tbody>')
  for (const row of table.rows) {
    lines.push('<tr>')
    for (const cell of row.cells) lines.push(`<td>${escapeHtml(cell)}</td>`)
...
function renderSectionBody(section: Section): string[] {
  ...
  if (section.claims.length === 0) return ['<p><em>(항목 없음)</em></p>']
  return ['<ul>', ...section.claims.map(renderClaimLi), '</ul>']
}

/** 한 섹션 -> <h2> + 선택 prose + 본문. */
function renderSection(section: Section): string[] {
  const lines: string[] = [`<h2>${escapeHtml(section.heading)}</h2>`]
 ...
 */
export function exportHtml(doc: GeneratedDoc, meta: DocMeta): string {
  const lines: string[] = [
    '<!DOCTYPE html>',
    '<html lang="ko">',
    '<head>',
  ...
  for (const section of doc.sections) lines.push(...renderSection(section))
 ...
 */
export function exportVaultHtml(vault: WikiVault): WikiVault {
...
function wrapMarkdownAsHtml(path: string, markdown: string): string {
