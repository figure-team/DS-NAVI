...
export const ProgramInventorySchema = z.object({
    ...
    stats: z.object({
        ...
        byType: z.array(z.object({ type: ProgramTypeSchema, count: z.number().int().nonnegative() })),
        /**
        ...
        excluded: z.object({
            ...
            otherLang: z.array(z.object({ lang: z.string(), count: z.number().int().nonnegative() })),
            /** 판독 실패 파일 수(loc=0 처리). */
...
/** 간이법 평균복잡도 가중치(미조정). */
export const FP_WEIGHTS = { ei: 4.0, eo: 5.2, eq: 3.9, ilf: 7.5, eif: 5.4 };
/** 파일 basename(확장자 제거). */
...
export function buildProgramInventory(projectRoot, inputs) {
    ...
    for (const j of inputs.batchJobs?.jobs ?? []) {
        for (const f of new Set([j.handlerFile, j.evidence.file])) {
            if (!f || !f.endsWith('.java'))
                continue;
            const set = batchFiles.get(f) ?? new Set();
            set.add(j.id);
    ...
    for (const c of [...(inputs.candidates?.candidates ?? [])].sort((a, b) => cmp(a.key, b.key))) {
        ...
        for (const r of c.roots) {
            if (!domainByFile.has(r))
                domainByFile.set(r, { domain: c.key, via: 'reachability' });
        }
        for (const f of c.files) {
            if (!domainByFile.has(f.relPath))
                domainByFile.set(f.relPath, { domain: c.key, via: f.via });
        }
    }
    ...
    for (const f of census.files) {
        ...
        if (isTest) {
            ...
            type = 'screen';
            for (const id of screenRoutes.get(f.relPath) ?? [])
                notes.add(id);
            // 이중 컨트롤러(form+api): api routeId 도 notes 에 보존(추적성 유실 방지).
            if (apiRoutes.has(f.relPath)) {
                notes.add('also:api');
                for (const id of apiRoutes.get(f.relPath) ?? [])
                    notes.add(id);
            }
        }
            ...
            type = 'batch';
            for (const id of batchFiles.get(f.relPath) ?? new Set())
                notes.add(id);
        }
        else if (isMapperXml) {
            type = 'mapper-xml';
    ...
    const transactions = [...routes.routes]
    ...
    const unadjustedFp = Math.round((ei * FP_WEIGHTS.ei + eq * FP_WEIGHTS.eq + ilf * FP_WEIGHTS.ilf + eif * FP_WEIGHTS.eif) * 10) / 10;
