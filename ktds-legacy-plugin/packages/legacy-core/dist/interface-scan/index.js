 ...
 */
function collectSuspects(projectRoot, census, detectedLines) {
    const samples = [];
    let count = 0;
    ...
    for (const relPath of javaFiles) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (/["']https?:\/\/[^"']+["']/.test(line))
                kind = 'http-literal';
            else if (/["']jdbc:[^"']+["']/.test(line))
                kind = 'jdbc-url';
            if (!kind)
                continue;
            if (detectedLines.has(`${relPath}:${i + 1}`))
                continue;
            count++;
            if (samples.length < SUSPECT_SAMPLE_CAP)
                samples.push({ file: relPath, line: i + 1, kind });
        }
    }
    ...
    for (const w of wsdls.sort((a, b) => cmp(a.relPath, b.relPath))) {
        count++;
        if (samples.length < SUSPECT_SAMPLE_CAP)
            samples.push({ file: w.relPath, line: 1, kind: 'wsdl-file' });
    }
...
export async function extractInterfaces(projectRoot, census, cache) {
    ...
    for (const c of cfgClients)
        customSpecs[c.type] = toInvocationSpec(c);
    const raw = [];
    const byLang = (lang) => census.files
    ...
    const sigSec = cache?.section('interface-signals', `${INTERFACE_SIGNALS_SALT}:cfg-${cfgHash}`);
    ...
    items.sort((a, b) => cmp(a.protocol, b.protocol) ||
    ...
    for (const item of items)
        protoCounts.set(item.protocol, (protoCounts.get(item.protocol) ?? 0) + 1);
    ...
    for (const item of items)
        for (const c of item.callSites)
            detectedLines.add(`${c.file}:${c.line}`);
