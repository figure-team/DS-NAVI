...
import { PolicySignalSetSchema, POLICY_SIGNALS_FILENAME, type PolicyCategory } from './types.js'
...
export function policyFillPrepDir(projectRoot: string): string {
  return join(specMapDir(projectRoot), POLICY_FILL_PREP_DIR)
}

/** `.spec/map/policy-fill-frag/` 디렉터리 경로. */
export function policyFillFragDir(projectRoot: string): string {
...
export async function readPolicyFillChunkIndex(projectRoot: string): Promise<PolicyFillChunkIndex> {
  let raw: string
  try {
...
// ──────────────────────────────────────────────────────────────────────────
// prep — fill unit 추출(모드별) → 문서별 청크
// ──────────────────────────────────────────────────────────────────────────

/** 모드 무관 채움 단위(중간 표현) — docId + 표시/원문 + 앵커. rowKey 는 부여 후 확정. */
interface FillUnit {
  docId: string
  title: string
  ...
  subject: string
  detail: string
  anchor: { file: string; line: number | null } | null
}
...
async function readPolicySignalSet(projectRoot: string) {
  let raw: string
  try {
...
async function categoryFillUnits(projectRoot: string): Promise<FillUnit[]> {
  ...
  return set.signals.map((s) => ({
    ...
    category: s.category as PolicyCategory,
    kind: s.kind,
...
function domainFillUnits(inputs: DomainPolicyInput[]): FillUnit[] {
  const units: FillUnit[] = []
  for (const d of inputs) {
    const docId = `policy-domain-${d.key}`
    const title = d.name
    for (const b of d.branches as BranchSignal[]) {
...
function assignRowKeys(units: FillUnit[]): (FillUnit & { rowKey: string })[] {
 ...
 */
export async function prepPolicyFill(
  projectRoot: string,
  options: PrepPolicyFillOptions = {},
): Promise<{ index: PolicyFillChunkIndex; paths: string[] }> {
  ...
  const byDoc = new Map<string, (FillUnit & { rowKey: string })[]>()
  for (const u of units) {
    const list = byDoc.get(u.docId)
    if (list) list.push(u)
    else byDoc.set(u.docId, [u])
  }
  const rowCmp = (a: FillUnit & { rowKey: string }, b: FillUnit & { rowKey: string }) =>
  ...
  const entries: PolicyFillChunkIndex['chunks'] = []
  const skippedDocs: PolicyFillChunkIndex['skippedDocs'] = []
  const paths: string[] = []
    ...
    for (let i = 0; i < docUnits.length; i += maxRows) {
      ...
      for (const u of group) {
        ...
        if (u.anchor && u.anchor.line !== null) {
          ...
          if (cur === undefined || u.anchor.line < cur) anchorByRel.set(u.anchor.file, u.anchor.line)
        }
        if (preCite === null) preCiteMissing++
        rows.push({
          rowKey: u.rowKey,
          docId,
          subject: u.subject,
  ...
  const index: PolicyFillChunkIndex = {
    schemaVersion: 1,
    gitCommit,
    mode,
    maxRows,
  ...
  await writeFile(
    join(prep, POLICY_FILL_PREP_INDEX_FILENAME),
    stableJson(PolicyFillChunkIndexSchema.parse(index)),
    'utf8',
  )
  return { index, paths }
}
...
async function currentGitCommit(projectRoot: string, mode: PolicyFillMode): Promise<string | null> {
  try {
    if (mode === 'category') {
 ...
 * `only` 로 부분 감사(스킵 가드용).
 */
export async function auditPolicyFillFragments(
  projectRoot: string,
  only?: string[],
): Promise<PolicyFragmentAudit> {
  ...
  for (const entry of index.chunks) {
    ...
    for (const r of frag.rows) {
      if (!declared.has(r.rowKey)) continue // 선언 밖 행은 병합에서 드랍(감사는 통과).
...
function stripFillSection(md: string): string {
...
/** 병합된 행(조각 채움 + 청크 사실) — 렌더용. */
interface MergedRow {
  subject: string
  statement: string
  confidence: PolicyFillTag
  citations: Citation[]
  anchorFile: string
...
function renderFillSection(rows: MergedRow[]): string {
  const header =
    `\n${FILL_SECTION_START}\n` +
    `## 규범 진술 (LLM 보강)\n\n` +
 ...
 */
async function verifyFragmentRow(
  projectRoot: string,
  row: PolicyFillFragmentRow,
  cache: Map<string, FileCache>,
): Promise<{ citations: Citation[]; confidence: PolicyFillTag; removed: number; demoted: boolean }> {
  const kept: Citation[] = []
  let removed = 0
  for (const c of row.citations) {
    const status = await verifyCitation(projectRoot, c, cache)
    if (status === 'ok') kept.push(c)
    else removed++
  }
...
export async function mergePolicyFillFragments(projectRoot: string): Promise<MergePolicyFillResult> {
  ...
  const factByKey = new Map<string, PolicyFillRow>()
  const rowsByDoc = new Map<string, MergedRow[]>()
  ...
  for (const docId of allDocIds) {
    ...
    if (!existsSync(path)) {
      ...
      if (rows && rows.length > 0) missingDocs.push(docId)
      continue
    }
    const original = await readFile(path, 'utf8')
    if (rows && rows.length > 0) {
      const sorted = rows.slice().sort(
        (a, b) => cmp(a.anchorFile, b.anchorFile) || (a.anchorLine ?? 0) - (b.anchorLine ?? 0) || cmp(a.subject, b.subject),
      )
