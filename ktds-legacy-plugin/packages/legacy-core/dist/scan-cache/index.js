...
function statKey(section, relPath) {
...
export class ScanCacheSession {
    ...
    putKeys = new Set();
    constructor(projectRoot, census, opts = {}) {
    ...
    section(name, salt) {
        ...
        if (nextSection && nextSection.salt !== salt) {
            throw new Error(`[scan-cache] 섹션 '${name}' 을 서로 다른 salt 로 재오픈(${nextSection.salt} → ${salt}) — 공유 섹션은 동일 salt 상수를 import 해 쓰세요.`);
        }
        if (!nextSection) {
        ...
        return {
            get: (relPath) => {
                ...
                if (fresh !== undefined)
                    return structuredClone(fresh.value);
     ...
     */
    finalize() {
        const out = { schemaVersion: SCAN_CACHE_SCHEMA_VERSION, sections: {} };
        const names = new Set([...this.next.keys()]);
        for (const name of Object.keys(this.prev?.sections ?? {}))
            names.add(name);
        for (const name of [...names].sort(cmp)) {
            ...
            if (observed) {
                for (const [rel, entry] of observed.entries)
                    merged.set(rel, entry);
            }
            if (merged.size === 0)
                continue;
            const entries = {};
            for (const rel of [...merged.keys()].sort(cmp))
                entries[rel] = merged.get(rel);
...
export function createScanCacheSession(projectRoot, census, opts = {}) {
