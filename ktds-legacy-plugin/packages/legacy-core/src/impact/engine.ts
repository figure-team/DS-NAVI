...
export function loadImpactInputs(projectRoot: string): ImpactInputs {
...
export function loadKgTableCatalog(projectRoot: string): KgTableEntry[] {
  ...
  for (const n of g.nodes ?? []) {
    ...
    const lr =
      Array.isArray(n.lineRange) && n.lineRange.length === 2 ? (n.lineRange as number[]) : null
    out.push({
      name: n.name,
      filePath: n.filePath,
...
function extractMapperNamespace(xml: string): string | null {
  const m = xml.match(/<mapper\b[^>]*\bnamespace\s*=\s*["']([^"']+)["']/)
  return m ? m[1] : null
}

/** 매퍼 XML(엣지 target)을 읽어 namespace·라인수 인덱스 산출(IO). */
export function buildMapperInfo(
  projectRoot: string,
  edges: EdgesReport['edges'],
): { mapperNamespaceByPath: Map<string, string>; mapperLineCounts: Map<string, number> } {
  ...
  for (const rel of [...targets].sort(cmp)) {
    try {
      ...
      if (ns !== null) mapperNamespaceByPath.set(rel, ns)
    } catch {
...
export function buildImpactReport(
  ...
  if (allowed.has('import')) {
    ...
    const upI = reachClosure(
      seedPaths,
      buildAdjacency(inputs.edges.edges, withImport, 'reverse'),
      options.depthCap,
    )
    const dnI = reachClosure(
      seedPaths,
      buildAdjacency(inputs.edges.edges, withImport, 'forward'),
      options.depthCap,
    )
...
export function buildClaimItems(result: ImpactResult): ImpactClaimItem[] {
...
export function fillClaimSnippets(projectRoot: string, items: ImpactClaimItem[]): void {
  const rootAbs = resolve(projectRoot)
  const cache = new Map<string, string[] | null>()
  for (const item of items) {
    for (const c of item.citations) {
      const abs = resolve(projectRoot, c.filePath)
      if (abs !== rootAbs && !abs.startsWith(rootAbs + sep)) continue // path-escape(어휘적)
...
export function analyzeImpact(
  ...
  const impactPath = writeMapArtifact(
    projectRoot,
  ...
  const verifyPath = writeMapArtifact(
    projectRoot,
