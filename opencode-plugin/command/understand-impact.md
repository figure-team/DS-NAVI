...
1. 카탈로그를 받는다:
   ```
   node $ATLAS_PLUGIN_ROOT/scripts/understand-impact.mjs <projectRoot> seeds
   ```
...
확정된 시드로:
```
node $ATLAS_PLUGIN_ROOT/scripts/understand-impact.mjs <projectRoot> analyze --path <파일> [--path <파일2> ...]
```
...
1. **선례검색 (F1):** 의도에서 신호(도메인 힌트·엔티티·연산)를 뽑아 엔진에 넘긴다:
   ```
   node $ATLAS_PLUGIN_ROOT/scripts/understand-impact.mjs <projectRoot> precedents --domain <힌트> [--entity <명사>] [--op <연산>] [--top N]
   ```
...
3. **3분류 발행:** 선택된 선례 + 변경 대상 기존 파일로:
   ```
   node $ATLAS_PLUGIN_ROOT/scripts/understand-impact.mjs <projectRoot> analyze \
