...
function findWebInfXmls(projectRoot: string, maxDepth = 8): string[] {
  const out: string[] = []
  const walk = (rel: string, depth: number) => {
    ...
    for (const name of entries) {
      if (WALK_SKIP.has(name) || name.startsWith('.')) continue
      ...
      if (st.isDirectory()) walk(childRel, depth + 1)
      else if (name.endsWith('.xml') && childRel.includes('WEB-INF/')) out.push(childRel)
    }
  }
  walk('', 0)
  return out.sort()
...
function extractAttr(block: string, name: 'prefix' | 'suffix'): string | null {
...
export function loadViewResolverConfigs(projectRoot: string): ViewResolverConfig[] {
  ...
  for (const rel of findWebInfXmls(projectRoot)) {
    ...
    if (!text.includes('ViewResolver')) continue
    ...
    for (const block of text.split(/<bean[\s>]/).slice(1)) {
      ...
      if (!/class="[^"]*ViewResolver"/.test(body)) continue
...
export function resolveViewName(
  viewName: string,
  configs: ViewResolverConfig[],
  existsRel: (rel: string) => boolean,
): string | null {
  if (!viewName || viewName.includes(':')) return null
 ...
 */
export function extractReturnViewNames(sourceLines: string[], declLine: number): string[] {
  ...
  for (let i = open; i < sourceLines.length; i++) {
    ...
    for (const m of line.matchAll(/return\s+"([^"]+)"/g)) names.push(m[1])
    for (const m of line.matchAll(/ModelAndView\(\s*"([^"]+)"/g)) names.push(m[1])
    for (const ch of line) {
      if (ch === '{') depth++
      else if (ch === '}') {
        depth--
        if (depth === 0) return [...new Set(names)]
      }
    }
  }
...
function screenRoutePath(screenId: string): string | null {
  const raw = screenId.replace(/^screen:/, '').split('__')[0]
  if (!raw || raw === '(root)') return null
  return raw.startsWith('/') ? raw : `/${raw}`
}

export function resolveScreenViews(
  screens: Screen[],
  projectRoot: string,
): { screens: Screen[]; summary: ViewResolveSummary } {
  const configs = loadViewResolverConfigs(projectRoot)
  const summary: ViewResolveSummary = {
  ...
  const existsRel = (rel: string) => existsSync(join(projectRoot, rel))
  const routes = readMapArtifact(projectRoot, ROUTES_FILENAME, RoutesReportSchema)
  const routeByPath = new Map<string, { filePath: string; line: number }>()
  for (const r of routes?.routes ?? []) {
    if (!routeByPath.has(r.path)) routeByPath.set(r.path, { filePath: r.filePath, line: r.line })
  }
  // KG JSP 목록 — graphNodeId 는 KG 실존 시에만(부재 시 null 유지, SKILL 규약).
  let graphJsps: Set<string> | null = null
  try {
    const kg = JSON.parse(
      readFileSync(join(projectRoot, '.understand-anything', 'knowledge-graph.json'), 'utf8'),
    ) as { nodes?: unknown[] }
    graphJsps = new Set(listJspFilesFromGraph((kg.nodes ?? []) as never))
  } catch {
    graphJsps = null
  }
  ...
  const out = screens.map((s) => {
    ...
    if (jspFile && !existsRel(jspFile)) {
      ...
      if (route && lines) {
        const resolved = [
          ...new Set(
            extractReturnViewNames(lines, route.line)
    ...
    if (!jspFile) summary.unresolved++
    if (jspFile === s.jspFile) return s
    return {
      ...s,
      jspFile,
...
export function resolveScreenViewsOnDisk(projectRoot: string): {
  screensPath: string
  ...
  try {
    ...
    unmatchedJsps = reconcileJsps(
      listJspFilesFromGraph((kg.nodes ?? []) as never),
      screens,
      file.fragments,
    )
  } catch {
  ...
  const next: ScreensFile = ScreensFileSchema.parse({ ...file, screens, unmatchedJsps })
