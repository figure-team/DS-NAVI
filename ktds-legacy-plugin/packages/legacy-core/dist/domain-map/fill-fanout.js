...
export function fillPrepDir(projectRoot) {
    return join(specMapDir(projectRoot), FILL_PREP_DIR);
}
/** `.spec/map/fill-frag/` 디렉터리 경로. */
export function fillFragDir(projectRoot) {
...
export async function readFillChunkIndex(projectRoot) {
    let raw;
    try {
...
async function readBundles(projectRoot) {
    ...
    if (names.length === 0)
        throw new Error('bundle/ 이 비어 있음 — 먼저 bundle 을 실행하세요');
    const bundles = [];
 ...
 */
export async function prepFillChunks(projectRoot, options = {}) {
    ...
    for (const bundle of bundles) {
        ...
        const flowPreCites = new Map();
        for (const flow of bundle.flows) {
            flowPreCites.set(flow.flowId, flow.filePath ? await extractPreCite(projectRoot, flow.filePath, flow.line, fileCache) : null);
        }
        ...
        for (let gi = 0; gi < groups.length; gi++) {
            ...
            for (const relPath of relPaths) {
                ...
                let slice = bundleFile?.slice ?? (await sliceFile(projectRoot, relPath, anchorLine, DEFAULT_SLICE_LINES));
            ...
            paths.push(filePath);
            const preCiteMissing = flows.filter((f) => f.preCite === null).length + steps.filter((s) => s.preCite === null).length;
            entries.push({
                chunkId,
                domainId: bundle.domainId,
                key: bundle.key,
    ...
    await writeFile(join(prep, FILL_PREP_INDEX_FILENAME), stableJson(FillChunkIndexSchema.parse(index)), 'utf8');
    return { index, paths };
}
/**
 ...
 */
export async function auditFillFragments(projectRoot, only) {
