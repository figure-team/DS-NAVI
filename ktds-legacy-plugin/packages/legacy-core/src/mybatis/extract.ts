...
function tablesInSql(sql: string): string[] {
  ...
  const lastSeg = (t: string) => (t.split('.').pop() ?? t).trim()
  // FROM a, b, c  (서브쿼리 "FROM (" 는 ident 불일치로 자연 제외)
  for (const m of sql.matchAll(new RegExp(`\\bFROM\\s+(${qident}(?:\\s*,\\s*${qident})*)`, 'gi'))) {
    for (const t of m[1].split(',')) out.push(lastSeg(t))
  }
  for (const m of sql.matchAll(new RegExp(`\\bJOIN\\s+(${qident})`, 'gi'))) out.push(lastSeg(m[1]))
  for (const m of sql.matchAll(new RegExp(`\\bINTO\\s+(${qident})`, 'gi'))) out.push(lastSeg(m[1]))
  for (const m of sql.matchAll(new RegExp(`\\bUPDATE\\s+(${qident})`, 'gi'))) out.push(lastSeg(m[1]))
