...
- **스코프 구분 (2026-06-15 사용자 지시):** 근시일 = **공통 계약 + 프로파일 R(`/understand-docs` 참조 샤드) + 방법론 모듈 시스템(Layer 1·2, D1~D4, D7, §2-A)**. **보류 = AIDD 위임구현 기능 = 프로파일 W(변경 스토리) + 위임구현 게이트(D5) + 출처 기반 근거 게이트(D6) + 출처 각인·STALE 게이트·W 생성 위치(D8) + 다중 변경 격리·겹침 승인 차단(D9)** — 신설 기능 착수 시점에 진행. 본 ADR은 보류분의 설계 기록을 미리 확정해 둔다(통일 계약이 R/W를 함께 규정해야 일관되므로).
...
### D9 — 다중 변경 요청 격리 + 생애주기 + 교차 staleness + 겹침 승인 차단 (사용자 확정)

여러 변경을 따로 요청할 때(기능 A·B 병렬 진행) 서로 덮어쓰거나 충돌하지 않도록:

1. **격리 키 = 스토리ID** (외부 SR-ID 있으면 연계). 각 변경은 자기 키로 격리 — impact 아카이브(`.spec/impact/<key>/`, ADR-002 부록 A.2)·W 스토리·대시보드 오버레이가 이 키로 묶인다. A·B 서로 무간섭. impact.json **최신 단일 슬롯**(last-writer-wins)은 대시보드 편의용일 뿐 — **스토리는 키-고정 impact 스냅샷에 바인딩**(D8 스탬프가 최신 슬롯이 아니라 키 아카이브를 가리킴) → B 재분석이 A 스토리를 건드리지 않는다.
2. **독립 생애주기.** 각 W 스토리는 자기 doc-state(DRAFT→APPROVED→구현중→완료). 완료분 아카이브, 미완은 `status --list`로 병렬 추적.
3. **교차 staleness (D8 일반화).** A를 먼저 구현→코드 변경→B 스탬프 무효화→B는 구현 직전 STALE 게이트에 걸려 자동 재생성. "먼저 머지된 변경이 나머지 in-flight 스토리를 무효화" → 순서가 꼬여도 안전.
4. **겹침 = 승인 차단 (fail-closed, 사용자 확정).** 스토리 승인 시 그 **대상파일(구현 쓰기 집합)**이 다른 미완 스토리(DRAFT/APPROVED/구현중 — ≠완료/RETURNED)의 대상파일과 겹치면 **승인 거부**(두 AI 구현이 같은 파일에서 경합하는 사고 방지). 해제: 겹치는 선행 스토리를 완료/반려하거나 대상에서 제외. `--force`는 강제 승인하되 감사에 `forced`+겹침 스토리ID 표기(D6 approve `--force`와 동일 패턴).
   - **차단 기준 = 대상파일(쓰기) 겹침만.** 영향집합(읽기/전파, hub 경유) 겹침은 **경고만** — 공용 유틸 hub 경유 false 겹침으로 거의 모든 변경이 차단되는 것을 방지(impact 과도전파 제어 ID5 정합).
