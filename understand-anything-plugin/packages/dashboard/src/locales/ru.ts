export const ru = {
  ...
  structure: {
    menuTitle: "СТРУКТУРА",
    root: "Структура",
    breadcrumbLabel: "Хлебные крошки структуры",
    crossDomainUnavailable: "Данные о зависимостях недоступны — повторите /understand-map scan",
    noCrossGroupEdges: "Нет междоменных зависимостей",
    evidenceTitle: "Основания зависимости",
    evidenceWeight: "{count} файловых связей",
    evidenceClose: "Закрыть",
    evidenceMore: "ещё {count}",
    bfNodeCount: "{count} узлов",
  ...
  impactAnalyze: {
    title: "Исследование влияния (естественный язык)",
    description:
      "Опишите планируемое изменение на естественном языке. Сначала будут предложены кандидаты в сиды; анализ выполняется только по подтверждённым сидам. Результаты записываются в меню «Влияние изменений» (и в оверлей графа структуры).",
    intakeHint:
      "Это исследовательский анализ — для официального приёма запроса и декомпозиции требований используйте Трассировку → Сессии запросов.",
    placeholder: "напр.: Хочу добавить вход через Kakao",
    run: "Предложить кандидатов в сиды",
    submitting: "Запуск…",
    cancel: "Отмена",
    running: "Исследование влияния",
    runningHint: "Исследование влияния выполняется — нажмите, чтобы открыть",
    ...
    modelDefault: "Модель сессии (по умолчанию)",
    modelAria: "Выбрать модель анализа",
    toastDone: "Анализ влияния завершён — результаты записаны в меню «Влияние изменений».",
    toastFailed: "Анализ влияния не удался. Проверьте консоль/логи.",
    candidatesTitle: "Подтверждение кандидатов в сиды",
    candidatesDesc:
      "Кандидаты в сиды, предложенные по вашему запросу. Подтвердите файлы для анализа — сиды определяете вы, а не модель.",
    candidatesEmpty: "Кандидаты не предложены. Уточните запрос или добавьте пути вручную.",
    candidatesLoadFailed: "Не удалось загрузить кандидатов в сиды.",
    candidatesFor: "Запрос",
    addPathPlaceholder: "Добавить вручную: путь относительно проекта",
    addPath: "Добавить",
    analyzeRun: "Анализировать с этими сидами",
    newQuery: "Новый запрос",
    toastCandidates: "Кандидаты в сиды готовы — подтвердите их, чтобы запустить анализ.",
  },
  layer: {
    defaultName: "Слой",
    label: "слои",
  ...
  onboarding: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "В верхней панели также есть Filter (фильтр по типу / сложности), Export (экспорт графа), Path (поиск пути между двумя узлами) и Theme (смена темы). ",
