...
export function buildApiSpec(input: DocInput): GeneratedDoc {
  // 엔드포인트: endpoint/route 태그 노드(노드 근거 승계).
  const endpoints = nodesWithTag(input.nodes, 'endpoint', 'route').map((n): Claim =>
    nodeClaim(n, `엔드포인트: ${displayName(n)}${summarySuffix(n)}`),
  )

  // 라우팅/미들웨어: routes 리포트의 각 라우트는 file:line 근거 -> CONFIRMED.
  const routing = sortedRoutes(input).map((r): Claim => {
