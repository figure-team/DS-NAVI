...
> (좌 270px 세션 원장 + 우 스테퍼·산출물 — `RTM_INTAKE_WORKSPACE_DESIGN.md` §2.2). 이 탭은 위 "전치"
...
2. `claude -p` 가 판정:
   ...
   - **기존 도메인 수정?**(EX2 결제) → 도메인 인벤토리에서 매칭 → 그 도메인 **안에** 행 추가 +
     ...
     **미구현** — 코드영향 검증(P6)이 아직 진행 중이라 이 산출 경로는 배선되지 않았다
     (`RTM_IMPACT_GATE_DESIGN.md` §9 P6). 현재 ①식별은 근거 번들의 6축 요약만 인용한다(P5).
