...
function sortedTsLikePaths(census) {
    return census.files
 ...
 */
async function buildTsApiCallEdges(projectRoot, census, routes) {
...
export async function extractRoutes(projectRoot, census, cache) {
    ...
    const routeKtSec = cache?.section('spring-routes-kotlin', KOTLIN_SPRING_ROUTES_SALT);
    ...
    const parsedKt = new Map();
    const cachedKtByPath = new Map();
    for (const f of kotlinFiles) {
        const hit = routeKtSec?.get(f.relPath);
        if (hit !== undefined) {
            if (hit !== null)
                cachedKtByPath.set(f.relPath, hit);
            continue;
        }
        try {
            const src = readFileSync(join(projectRoot, f.relPath), 'utf8');
    ...
    const sortedKotlin = [...new Set([...parsedKt.keys(), ...cachedKtByPath.keys()])].sort((a, b) => a < b ? -1 : a > b ? 1 : 0);
    ...
    const resolvedBatch = resolveBatchHandlers(batchEntries, beanIndex, census).filter((b) => !isTestPath(b.filePath));
    return {
        schemaVersion: 1,
