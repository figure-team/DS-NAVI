...
const uniqSort = (xs) => [...new Set(xs)].sort(cmp);
const TAG_CRUD = { select: 'R', insert: 'C', update: 'U', delete: 'D' };
/** 동적 태그/바인딩 파라미터/주석 제거 후 평문 SQL. */
function plainSql(body) {
    return body
        .replace(/<!--[\s\S]*?-->/g, ' ')
...
function columnsInSql(sql, crud) {
    ...
    if (crud === 'C') {
        ...
        if (setIdx >= 0) {
            ...
            for (const m of region.matchAll(new RegExp(`(${ident})\\s*=`, 'g')))
                cols.push(m[1]);
        }
    }
 ...
 */
const MAPPER_ROOT_RE = new RegExp('^\\uFEFF?\\s*(?:(?:<\\?[^>]*\\?>|<!--[\\s\\S]*?-->|<!DOCTYPE[^>]*>)\\s*)*<mapper[\\s>]', 'i');
export function isMapperXmlDocument(content) {
    return MAPPER_ROOT_RE.test(content);
}
/** 한 Mapper XML 내용 → MyBatisMapper. 루트가 `<mapper namespace>` 가 아니면 null. */
export function parseMapperXml(content, relPath) {
...
export function buildMyBatisModel(files) {
    const mappers = [];
...
export function namespaceBaseName(namespace) {
