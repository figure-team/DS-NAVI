...
>
> **선행 설계:** `RTM_TAB_DESIGN.md`(§3 인테이크), `RTM_STEP_FLOW_DESIGN.md`(6단계 게이트),
...
> **아래는 개선 전(2026-07-16 P5 이전) 상태이고 번호도 그때의 5단계 체계다** — 지금은 ①이 6축
> 근거 번들을 읽고(P5) 코드영향은 ②로 갈렸다(§6.5). 문제 정의의 기록으로 보존한다.
...
①의 추측 위에 문서 4종과 정식 추적표 행이 선다. (신 체계로는 ⑥) RTM 반영 이전은 되돌릴 수 있지만
(`RTM_STEP_FLOW_DESIGN.md` §6), **틀렸다는 걸 알아챌 장치가 없다.**
...
① 식별  — "무엇을 바꾸나"
   ...
   4. identified.json 에 6축 근거 기록                          (신규 스키마)
   5. ★ 실재 대조(validate): changeset fnId ⊂ rtm.json / 테이블 ⊂ db-schema
        (신규, 결정론, fail-closed) ← 근거 없는 신규 단언 차단
        ※ ①의 **자기 검증**이자 ②의 입구 전제(fnId 실존 → 시드 조인 성립)
   6. 보고 후 멈춤 — ① 컨펌 게이트
        ↓ 컨펌
② 영향분석 — "그게 무엇을 건드리나"                          (6단계 승격, §6.5)
   1. ★ 코드영향 검증: changeset.modified → 시드 파일 → impact analyze (§6.3)
        시드 도출은 결정론 조인 — LLM 이 고르지 않는다
   2. 요청별 저장(impact-run.json) + impact 원장 기록(query=요청 원문)
   3. 보고 후 멈춤 — ② 컨펌 게이트(엔진 출력만이 근거. 산문 금지)
        ↓ 컨펌
③ 목록표 ▶ ④ 정의서 ▶ ⑤ 명세서   — 근거 + 영향 위에 쓰인다
        ↓
⑥ RTM 반영 — AC/changeset 이 GROUNDED 로 승격 가능
```

> **왜 ①과 ②가 갈렸나**: 게이트의 존재 이유가 "깊은 작업 전에 얕은 범위 확정"인데 초판 배치는
> ①이 가장 깊은 작업(impact BFS)을 품게 했다 — §6.5 참조(반려 철회 경위 포함).
...
### 6.5 왜 6번째 단계로 만들지 않나 — **반려 철회. 6단계로 승격했다(2026-07-16)**

> **초판의 반려 근거가 사실과 반대였다. 경위를 남긴다 — 같은 실수를 반복하지 않기 위해.**
>
> 초판은 이렇게 썼다:
>
> > ①~⑤ 번호는 외부 가이드(`templates/요구사항_작성순서_가이드.md`, study/ — 미벤더링)의 5단계에
> > 앵커돼 있고 … 6단계로 늘리면 가이드와 어긋나고 `STEP_DEFS`·`CIRCLED`·`targetStep`·세션 스키마·
> > URL·서버 게이트·스테퍼가 연쇄로 바뀐다. **"무엇이 영향받는지 식별"은 의미상 ①식별에 속한다.**
>
> **그 가이드를 읽지 않고 썼다**("미벤더링"이라 적어 놓고 근거로 삼았다). 읽어 보니 정반대였다:
>
> | 초판의 주장 | 가이드의 실제 내용 |
> |---|---|
> | ①~⑤가 가이드에 앵커돼 있고 6단계는 가이드와 어긋난다 | 맞다 — 다만 **코드영향은 애초에 가이드에 없다.** 가이드 전문에 "영향"·"impact" 언급 **0건** |
> | "무엇이 영향받는지 식별"은 의미상 ①식별에 속한다 | 가이드의 ①은 **"고객 인터뷰로 모호함 제거"**(담당 `PM/PL`, 질문 *"진짜 원하는 게 뭔가"* — 병행/대체? 동의항목? 계정연동?). **코드영향 엔진 실행은 성격이 전혀 다르다** |
>
> 즉 **가이드 앵커를 지키는 길은 ①에 코드영향을 넣는 게 아니라 ①에서 빼는 것**이었다.
> `04_과업내용변경요청서`·`05_변경영향분석서` 템플릿도 가이드에 **없고** 이 프로젝트가 만든 것이며
> `05` 헤더가 스스로 **"절차 B"**라 못 박는다 — 절차 A 의 코드영향은 어느 쪽에서도 선례가 없는
> **신규 개념**이다.
>
> **교훈: 외부 문서를 반려 근거로 삼으려면 먼저 읽어라.** "미벤더링"은 안 읽어도 되는 이유가
> 아니라 읽어야 할 이유다(§5.2 "이 버그는 전 지점을 grep 해야 한다"와 같은 계열의 교훈 —
> 확인하지 않은 전제를 근거로 쓰지 마라).

**결정(2026-07-16 사용자): 6단계로 승격.** `①식별 → ②영향분석 → ③목록표 → ④정의서 → ⑤명세서 → ⑥RTM`.

**결정적 근거는 가이드 정합이 아니라 게이트의 존재 이유다.** `RTM_STEP_FLOW_DESIGN.md` §2 가
단계 게이트를 *"깊은 작업 전에 얕은 범위를 사용자가 먼저 확정"*이라 규정하는데,
**초판 배치는 ①이 파이프라인에서 가장 깊은 작업(impact 엔진 BFS)을 품게 만들었다** — 게이트가
지키려던 바로 그 원칙을 ① 스스로 위반한 것이다. ①과 ②를 가르면 각 게이트가 자기 계약만 받는다:

| | ① 식별 | ② 영향분석 |
|---|---|---|
| 묻는 것 | *무엇을 바꾸나* — 요청 → 요구사항 분해 | *그게 무엇을 건드리나* — 코드 도달성 |
| 입력 | 근거 번들(6축) | ①의 `changeset.modified` |
| 출구 | `validate`(**자기 검증** — 내가 쓴 산출물이 스키마·실재에 맞나) | `code-impact`(**새 사실 생산**) |
| 산출 | `identified.json` | `impact-run.json` |

**`validate` 가 ①에 남는 이유**: 실재 대조는 ①이 **생산한 것의 무결성 검사**이지 새 지식이 아니다.
동시에 그것이 ②의 입구 전제다 — `modified` 의 fnId 실존이 보장돼야 시드 조인이 성립한다.
즉 `validate` 는 ①의 출구 조건이자 ②의 선행 조건이라 **①에 두는 것이 인과에 맞다.**

연쇄 변경(`STEP_DEFS`·`CIRCLED`·`targetStep`·세션 스키마·서버 게이트·스테퍼)은 초판 지적대로
실재했고 전부 수행했다 — **하위호환은 세션 마이그레이션으로 처리한다**(`RTM_STEP_FLOW_DESIGN.md` §4.3).
...
놓이는 자리는 **①(분해) 바로 뒤, 문서(③④⑤) 앞** — 즉 **②**다(6단계 승격 후, §6.5).
요청(REQ 원문)은 고객이 말한 것이니 먼저 존재하지만,
**요구사항(SFR/SIR·AC·changeset)은 분석 산출물 기반이어야 한다.**
이 절의 논지는 6단계화 이후에도 그대로다 — 오히려 **분해(①)와 영향(②)의 선후가 화면에서도 명시적**이
되어 닭-달걀의 해소 순서가 눈에 보인다.
...
| **C2** | **"완료" 판정 기준** — 매니페스트가 없다. 커밋 5종 공존을 어디까지 허용? | **전부 확정(2026-07-16 사용자 결정, §10)** — ① **축소 모드**: 최소집합(도메인·데이터·추적표)만 요구, 나머지는 있으면 포함·없으면 생략+명시. ② **최소집합 부재 시 fail-closed 차단**. ③ **커밋 불일치는 차단 않고 경고 + 낡은 축 결론 `[추정]` 강등**. ④ **생략된 축에 의존하는 결론도 `[추정]` 강등** |
...
| **C6** | **added 생성예측** — `--precedent/--entity` 필요, 선례 선택은 F2 사람 게이트(`SKILL.md:52` "최고점 자동채택 금지"), 선례 없으면 파일명 금지(L1). `CreationSuggestion` 은 impact.json 이 아니라 **md 09** 에만 발행 | **1차 범위 제외 확정**(2026-07-16, §10-3). modified 검증만으로 큰 축(근거 없는 "기존 flow 3개가 바뀐다" 단언)은 해소되고, F2 사람 게이트가 무인 실행과 구조적으로 충돌한다 |
...
| **C10** | **실행 시간** — ①은 번들생성+LLM, ②는 엔진(+팬아웃)으로 갈렸다(6단계, §6.5) — 단계당 부담이 줄었다 | 실측 후 판단 |
...
| **P0b** ✅ | **남은 스탬프 공백**(§9.1) — `understand-policy.mjs:200` 도메인 모드 `sourceCommit: null` **하드코딩**(6종). **결정됨(2026-07-16)**: `gitCommitHash(projectRoot)` = **생성 시점 HEAD**. 근거 — 도메인 모드는 `branch-scanner` 로 **소스를 직접 읽으므로** HEAD 가 실제 유래이고, 형제인 카테고리 모드도 사실상 같은 계보(`signals.gitCommit` ← `gitCommitHash`)이며 `db-schema`·`jpa`·`signal-scanner` 가 모두 이 관례다 | 재생성 후 6종 non-null | 小 |
...
| **P5** ✅ | **① 배선** — 번들 주입 + 실재대조 + `identified.json` 근거 기록 | 인테이크 e2e(라이브 claude), REQ-001 재실행 → evidence>0 — **완료 2026-07-16**: evidence 0→20, OAUTH_ACCOUNT 미발명, 평문 password 미결 인지, screenRefs 지목 | **大** |
| **P6** ✅ | **코드영향 검증**(C3,C9) — flow→시드 결정론 조인 + 요청별 impact 저장. **2026-07-16: 독립 단계 ②로 승격**(§6.5 — ① 안에 두려던 초판 배치를 철회) | `/change` 슬롯 무오염 확인 포함 | **大** |
| **P7** | **완료 게이트**(C2) — 최소집합 판정 + 부재 시 **fail-closed 차단** + 커밋 불일치는 **경고 + 낡은 축 강등** + `stale/` 배선(§5.3, 엔진은 이미 있고 호출자만 0) | 최소집합 없이 인테이크 시도 → 차단 · 커밋 어긋난 축의 결론이 `[추정]` 인가 | 中~大 |
| ~~**P8**~~ | ~~added 생성예측~~ — **범위 밖**(C6, 2026-07-16 결정) | — | — |
| **P9** ✅ | **UI** — ① 컨펌 화면에 6축 근거 표시, ② 컨펌 화면에 코드영향. 경계 문구 갱신 | 시각 QA(headless) — 6단계 스테퍼·세션 마이그레이션 실측 완료 | 中 |
| **P10** | **문서 정합** — `RTM_TAB_DESIGN`·`RTM_STEP_FLOW_DESIGN`·`PIPELINE_ORDER` 드리프트 해소. **6단계 승격분은 반영 완료**(§6.1·§6.5·§6.6) | 문서 리뷰 | 小 |

**P0~P6·P9 는 완료(2026-07-16).** **P1 도 독립적**이며 되돌릴 게 없다. **P5 는 라이브 e2e까지
통과했다**(위 표 참조). **남은 위험은 P7**이다 — 완료 게이트(최소집합 판정·stale 배선)가 아직이다.
...
## 10. 열린 질문 — **전부 결정됨(2026-07-16)**

1. ~~**"분석 완료" 의 최소집합은?**~~ → **축소 모드**.
   *"모든 분석이 완료될 필요는 없고 최소집합만 있으면 돌리되, 없으면 넘어가고 있으면 포함하는 게 맞다."*
   ...
   - **최소집합 부재 시**: **fail-closed 차단**. `understand-rtm/SKILL.md:45` 가 이미 "rtm.json 없으면
     멈춤"이라 규정했으나 코드에 미구현인 것을 구현하는 셈이다(현재는 조용히 빈 인벤토리로 진행해
     도메인 귀속이 전부 새 `to-be:` 로 떨어진다 — `rtm-intake.mjs:167-180`).
   - **축 생략 시**: 그 축에 의존하는 결론을 `[추정]` 강등(근거↔신뢰도 불변식과 일관).
2. ~~**커밋 불일치 허용 범위는?**~~ → **차단하지 않는다. 경고 + 낡은 축 결론 `[추정]` 강등.**
   실측이 이미 5종 공존이라 전부 일치를 요구하면 eGov 6,101파일 재분석을 매번 강제하게 된다.
   축소 모드 철학(있으면 포함·없으면 명시)의 연장 — **낡았으면 강등**.
3. ~~**added(신규) 근거를 1차 범위에 넣나?**~~ → **제외**(C6). modified 검증만으로 큰 축이 해소되고,
   선례 선택이 F2 사람 게이트라 무인 실행과 구조적으로 충돌한다.
