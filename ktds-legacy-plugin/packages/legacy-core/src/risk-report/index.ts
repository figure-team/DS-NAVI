...
export const RiskReportSchema = z.object({
  ...
  stats: z.object({
     ...
     */
    complexityUnmeasured: z.array(
      z.object({ ext: z.string(), count: z.number().int().nonnegative() }),
    ),
...
function countBelow(sorted: readonly number[], target: number, inclusive: boolean): number {
  let lo = 0
  let hi = sorted.length
  while (lo < hi) {
    const mid = (lo + hi) >> 1
    if (sorted[mid] < target || (inclusive && sorted[mid] === target)) lo = mid + 1
    else hi = mid
  }
  return lo
}
 ...
 */
function percentileRanks(values: ReadonlyArray<number | null>): {
 ...
 */
function assignGrades(sortedScores: readonly number[]): RiskGrade[] {
  ...
  let groupStart = 0
  for (let i = 0; i < n; i++) {
    if (i > 0 && sortedScores[i] !== sortedScores[i - 1]) groupStart = i
...
export async function buildRiskReport(
  ...
  const degenerate = new Set<RiskMetricKey>()
  if (normLoc.degenerate) degenerate.add('loc')
  if (normComplexity.degenerate) degenerate.add('complexity')
  if (normFanIn.degenerate) degenerate.add('fanIn')
  if (normFanOut.degenerate) degenerate.add('fanOut')
  if (normChurn.degenerate) degenerate.add('churn')
  ...
  const items: RiskItem[] = rows.map((r, i) => {
    ...
    const scored: Array<[RiskMetricKey, number]> = []
    const push = (k: RiskMetricKey, v: number | null) => {
      if (v !== null && !degenerate.has(k)) scored.push([k, v])
    }
    ...
    const notes = [...r.notes]
    if (scored.length === 0) notes.push('[미확인] 유효 지표 없음(전 지표 미측정/무분산)')
    const score =
      scored.length === 0
        ? 0
    ...
    return {
      ...
      normalized,
      score,
      grade: '하' as RiskGrade, // 정렬 후 상대 밴드로 재부여(아래).
      factors,
  ...
  for (let i = 0; i < items.length; i++) items[i].grade = grades[i]

  // 복잡도 미측정 확장자 분해(C8) — kotlin 처럼 문법 미탑재 1급 언어의 침묵 누락 방지.
  const unmeasuredByExt = new Map<string, number>()
  for (const it of items) {
    if (it.metrics.complexity !== null) continue
