...
const keyOf = (category, subject) => `${category}::${norm(subject)}`;
/** 정렬 안정성용 status 우선순위(위반→미정의→문서에만→준수). */
const STATUS_RANK = { 위반: 0, 미정의: 1, 문서에만: 2, 준수: 3 };
/** 문서 항목 × 신호 → 대조 결과(순수, 결정론). */
export function reconcilePolicy(items, signals, gitCommit = null, seedUnresolved = []) {
    ...
    entries.sort((a, b) => cmp(a.category, b.category) ||
...
function categoryOfFile(stem) {
    const s = stem.replace(/^policy-/, '');
    return PolicyCategorySchema.safeParse(s).success ? s : null;
}
/**
 * `.understand-anything/policy-input/*.md` 의 기존 정책서를 ingest·대조(IO 래퍼).
 * 입력 디렉터리가 없으면 빈 결과(대조 대상 없음 — "없을 때" 경로).
 */
export function scanPolicyReconcile(projectRoot, signals) {
    ...
    if (existsSync(dir)) {
        ...
        for (const f of files) {
            ...
            try {
                ...
                unresolved.push({ ref: f, reason: `ingest 실패: ${err.message}` });
            }
        }
    }
