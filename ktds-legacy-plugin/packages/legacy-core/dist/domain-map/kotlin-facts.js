...
import { collectDeclAnnotations, ktChild, ktChildren, ktDeclKind, ktImports, ktPackageName, ktTypeOuterName, } from './kotlin-ast.js';
/** 타입 노드 후보(직계 자식에서 타입으로 취급할 것들). */
const TYPE_NODE_TYPES = new Set(['user_type', 'nullable_type']);
/** 직계 자식 중 첫 타입 노드. */
function firstTypeChild(node) {
    for (const c of node.namedChildren) {
        if (c && TYPE_NODE_TYPES.has(c.type))
            return c;
    }
    return null;
}
/** 초기화식에서 타입 추정 — 단순 생성 호출(`Foo(...)`)만 인정, 그 외 null. */
function initializerTypeName(init) {
    if (!init)
        return null;
    if (init.type !== 'call_expression')
        return null;
    const callee = init.namedChildren.filter((c) => c !== null)[0];
...
function callInfo(node) {
    ...
    const argCount = (va ? va.namedChildren.filter((c) => c !== null).length : 0) + (hasLambda ? 1 : 0);
    if (callee.type === 'identifier') {
        return { methodName: callee.text, receiver: null, receiverText: null, argCount };
    }
    if (callee.type === 'navigation_expression') {
        const sub = callee.namedChildren.filter((c) => c !== null);
        if (sub.length < 2)
            return null;
        ...
        if (nameNode.type !== 'identifier')
            return null;
        const objNode = sub[0];
        const receiver = objNode.type === 'super_expression' ? { kind: 'super' } : exprToReceiver(objNode);
        return {
...
function isMergedInnerCall(node) {
    const p = node.parent;
    if (!p || p.type !== 'call_expression')
        return false;
    const first = p.namedChildren.filter((c) => c !== null)[0];
    ...
    const visit = (node) => {
        if (node.type === 'call_expression' && !isMergedInnerCall(node)) {
            ...
            if (vd) {
                ...
                const typeName = declared ?? initializerTypeName(init && init.id !== vd.id ? init : null) ?? 'var';
                if (nameId)
...
function returnTypeName(fn) {
    ...
    for (const c of fn.namedChildren) {
        ...
        if (afterParams && TYPE_NODE_TYPES.has(c.type))
            return ktTypeOuterName(c);
...
function ctorToMethod(ctor, className) {
...
function declToFact(decl, packageName) {
    const id = ktChild(decl, 'identifier');
    if (!id)
        return null; // 익명 object 표현식 등.
    const name = id.text;
    const kt = ktDeclKind(decl);
    const kind = kt === 'object' ? 'class' : kt;
    const fields = [];
    ...
    for (const body of bodies) {
        for (const prop of ktChildren(body, 'property_declaration')) {
            ...
            if (!nameId)
                continue;
            const declared = ktTypeOuterName(firstTypeChild(vd));
            const named = prop.namedChildren.filter((c) => c !== null);
            const init = named.length > 0 ? named[named.length - 1] : null;
            const typeName = declared ?? initializerTypeName(init && init.id !== vd.id ? init : null);
            if (!typeName)
                continue; // 타입 미상 프로퍼티는 싣지 않는다(미해소 정직 표기).
            fields.push({
                name: nameId.text,
                type: typeName,
                line: startLine(prop),
...
export async function extractKotlinFacts(relPath, src) {
...
//# sourceMappingURL=kotlin-facts.js.map