...
description: 가이드 1-명령 온보딩 — 신규 투입 개발자가 낯선 레거시 코드를 한 번에 분석(init→scan→자동 도메인 확정→문서 생성→커버리지 리포트). granular 명령은 파워유저용으로 유지.
...
## 0) 전제 (선택)
UA 대시보드/dual-load(예측·diff 시각화)를 원하면 **먼저** `/understand` 로 `knowledge-graph.json` 을 생성한다. ktds 결정론 분석(`.spec/map`)은 KG 없이도 동작하므로 온보딩 자체는 KG 없이 진행 가능하다.
...
4. **map** — 도메인 맵 요약(우선순위 랭킹 + 교차 도메인).
5. **docs** — 방법론 모듈(기본 `as-built`)로 5종 문서 + 위키 볼트 생성(모든 주장 `file:line` 근거).
6. **커버리지 리포트**(보완 D-c) — "분석이 코드의 몇 %를 정직하게 덮었나"(스캔 파일·계층 해소율·도달성·엣지 해소·cap 절단·비-Java 패스스루·JPA Tier C).
...
- **대시보드:** `/understand` 로 KG 생성 후 대시보드에서 흐름뷰·도메인 지도·영향도 시각화.
