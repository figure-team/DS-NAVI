 ...
 */
function dataCellFromRawSql(
  flow: UaGraphNode,
  input: DocInput,
  stepById: Map<string, UaGraphNode>,
): RtmTraceCell {
  const model = input.rawSqlModel
  if (isRawSqlModelEmpty(model)) return inferredCell()
 ...
 */
function testCell(flow: UaGraphNode, input: DocInput, stepById: Map<string, UaGraphNode>): RtmTraceCell {
  ...
  if (typeof flow.filePath === 'string') prodClasses.add(baseName(flow.filePath))
  for (const e of input.edges) {
    if (e.type !== 'flow_step' || e.source !== flow.id) continue
    const step = stepById.get(e.target)
    if (step && typeof step.filePath === 'string') prodClasses.add(baseName(step.filePath))
  }
  // 링크 수집(테스트 파일 단위 중복 제거 — 여러 클래스를 한 테스트가 참조해도 1건).
  const byTestFile = new Map<string, { testClass: string; line: number; convention: boolean }>()
  ...
  if (byTestFile.size === 0) return { value: '', confidence: 'UNVERIFIED', evidence: [] }
  ...
  const evidence: Evidence[] = entries.map(([file, v]) => ({ file, line: v.line }))
  const value = [...new Set(entries.map(([, v]) => v.testClass))].sort(cmp).join(', ')
  const confidence: Confidence = conventional.length > 0 ? 'CONFIRMED' : 'INFERRED'
  return { value, confidence, evidence }
}
