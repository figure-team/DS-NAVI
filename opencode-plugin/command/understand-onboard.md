...
## 0) 전제 (선택)
온보딩은 **대시보드 도메인 지도(`domain-graph.json`)까지 자동 생성**한다(아래 emit 단계). 구조 그래프 뷰·dual-load(예측·diff 시각화)를 원하면 별도로 `/understand` 로 `knowledge-graph.json` 을 생성한다(도메인 분석은 KG 없이도 동작).

## 1) 1-명령 온보딩
```
node $ATLAS_PLUGIN_ROOT/scripts/understand-onboard.mjs <projectRoot> [--by <담당자>] [--skip-docs] [--methodology as-built|si-standard]
```
체이닝 순서:
...
- **구조 그래프 뷰:** `/understand` 로 KG 생성 후 대시보드에서 구조 뷰·dual-load(도메인 지도는 onboard 가 이미 생성).
