 ...
 */
const FILE_NODE_TYPES = ['file', 'config', 'document', 'schema'];
...
export function buildKgNodeIndex(nodes) {
    const rank = (t) => {
        const i = FILE_NODE_TYPES.indexOf(t);
        return i < 0 ? FILE_NODE_TYPES.length : i;
    };
    // relPath → {rank, id} 최선 후보. rank 낮을수록(파일성) 우선, 동률은 id 사전순.
    const best = new Map();
    for (const n of nodes) {
        if (typeof n.id !== 'string' || typeof n.filePath !== 'string' || typeof n.type !== 'string')
            continue;
        const r = rank(n.type);
        // function/class 등 비-파일성 노드는 파일 대표가 될 수 없다 → 배제.
        if (r >= FILE_NODE_TYPES.length)
            continue;
    ...
    const idx = new Map();
    for (const [relPath, v] of best)
        idx.set(relPath, v.id);
    return idx;
}
/** impact.json 이 참조하는 모든 영향 relPath(시드 제외) — affected 후보. */
function affectedRelPaths(result) {
    const out = [];
    ...
    for (const s of result.upstream.persistence.sqlFiles)
        out.push(s.relPath);
    for (const t of result.upstream.persistence.jpaTables)
        out.push(t.relPath);
    return out;
}
/**
 ...
 */
export function buildImpactOverlay(result, kgIndex) {
...
export function loadKgNodeIndex(projectRoot) {
 ...
 */
export function writeImpactOverlay(projectRoot, overlay) {
 ...
 */
export function emitImpactOverlay(projectRoot, result) {
