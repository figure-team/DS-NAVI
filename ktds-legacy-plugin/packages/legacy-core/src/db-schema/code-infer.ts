...
import type { MyBatisMapper } from '../mybatis/types.js'
...
const minRef = (a: Ref, b: Ref): Ref =>
  cmp(a.relPath, b.relPath) < 0 || (a.relPath === b.relPath && a.line <= b.line) ? a : b
 ...
 */
export function inferTablesFromCode(
  projectRoot: string,
  census: CensusReport,
  ...
  const sec = cache?.section<MyBatisMapper | null>('mybatis-facts', MYBATIS_FACTS_SALT)
  const mappers: MyBatisMapper[] = []
  for (const f of census.files) {
    if (f.lang !== 'xml') continue
    const hit = sec?.get(f.relPath)
    if (hit !== undefined) {
      if (hit !== null) mappers.push(hit)
      continue
    }
    let source: string
    try {
      ...
      if (cache?.isAbsent(f.relPath)) sec?.put(f.relPath, null)
      continue
    }
  ...
  const anchor = new Map<string, Ref>() // 테이블 key → 최소 (relPath,line) 문 앵커.
  const nameOf = new Map<string, string>() // key → 표기 이름(추출기 대문자 보존).
  const colRefs = new Map<string, Map<string, Ref>>() // 테이블 key → 컬럼명 → 최소 앵커.
  for (const m of mappers) {
    for (const s of m.statements) {
      const ref: Ref = { relPath: m.relPath, line: s.line }
      for (const t of s.tables) {
        if (PSEUDO_TABLES.has(t)) continue
        const k = key(t)
        anchor.set(k, anchor.has(k) ? minRef(anchor.get(k)!, ref) : ref)
        if (!nameOf.has(k)) nameOf.set(k, t)
        if (s.tables.length === 1 && s.columns.length > 0) {
          let cols = colRefs.get(k)
          if (!cols) colRefs.set(k, (cols = new Map()))
          for (const c of s.columns) cols.set(c, cols.has(c) ? minRef(cols.get(c)!, ref) : ref)
        }
      }
    }
  }
  ...
  for (const k of [...anchor.keys()].sort(cmp)) {
    if (byKey.has(k)) continue // JPA 정의 우선.
    const ref = anchor.get(k)!
    const cols = [...(colRefs.get(k) ?? new Map<string, Ref>()).entries()].sort(([a], [b]) => cmp(a, b))
    byKey.set(k, {
      name: nameOf.get(k)!,
      relPath: ref.relPath,
      line: ref.line,
