...
argument-hint: ["[path] [--lite|--full|--auto-update|--no-auto-update|--review|--language <lang>]"]
---
...
- `$ARGUMENTS` may contain:
  - `--lite` — Deterministic-first analysis: every node (files, functions, classes, edges) is generated by script with template summaries. The only LLM dispatches are the Phase 1 project scanner (project narrative) and the Phase 4 architecture-layer analysis. Minutes instead of hours on large projects. No guided tour. **This is the default for projects that have never been analyzed.**
  - `--full` — Force a full LLM rebuild, ignoring any existing graph (per-file LLM summaries, semantic edges, guided tour)
...
6. Check if `$PROJECT_ROOT/.understand-anything/meta.json` exists. If it does, read it to get `gitCommitHash` and `analysisMode`.
7. **Mode resolution** — set `$MODE` to `lite` or `full` BEFORE the decision table:
   - `--lite` in `$ARGUMENTS` → `$MODE=lite`. If the existing meta has `analysisMode: "full"` (or an existing graph has no `analysisMode` — legacy graphs are full quality), warn the user once that this regenerates the graph with template summaries (downgrade) and proceed.
   - `--full` in `$ARGUMENTS` → `$MODE=full`.
   - Neither flag:
     - meta has `analysisMode: "lite"` → `$MODE=lite`
     - existing graph/meta present (with `analysisMode: "full"` or no `analysisMode` field) → `$MODE=full` — NEVER silently downgrade an LLM-quality graph
     - no existing graph or meta on disk → **check for a deleted-but-tracked graph first**: run `git -C "$PROJECT_ROOT" ls-files .understand-anything/knowledge-graph.json` (empty output or git failure = not tracked). If the graph IS tracked in git but missing from the working tree (common in demo/vendored repos that commit analysis artifacts), it was deleted, not never-created — do NOT silently default to lite over a restorable full graph. Ask the user: restore it (`git checkout -- .understand-anything/`), rebuild lite, or rebuild full. If non-interactive, STOP with that explanation instead of guessing.
     - otherwise (truly never analyzed) → `$MODE=lite` (**lite is the default for first-time analysis**)

8. **Decision logic:**

   | Condition | Action |
   |---|---|
   | `$MODE=lite` | **Lite analysis** — always a full regeneration (minutes-scale; no incremental path, no up-to-date prompt). Phases 1 → 1.5 → 2 (lite route) → 3 → 4 → skip 5 → 6 → 7. Phase 4 (architecture layers) is lite's ONE intentional LLM dispatch besides the Phase 1 scanner — layer identification is a judgment task; do NOT skip it, port layers from a previous graph, or substitute ad-hoc heuristics |
   | `$MODE=full` + `--full` flag | Full analysis (all phases) |
   | `$MODE=full` + `--review` flag + existing graph + unchanged commit hash | Skip to Phase 6 (review-only — reuse existing assembled graph) |
   | `$MODE=full` + existing graph + unchanged commit hash | Ask the user: "The graph is up to date at this commit. Would you like to: **(a)** run a full rebuild (`--full`), **(b)** run the LLM graph reviewer (`--review`), or **(c)** do nothing?" Then follow their choice. If they pick (c), STOP. |
   | `$MODE=full` + existing graph + changed files | Incremental update (re-analyze changed files only) |
...
9. **Collect project context for subagent injection:**
...
### Lite route (`$MODE=lite`)

No LLM dispatch at all. Report: `[Phase 2/7] Generating all nodes deterministically (--lite, no LLM)...`, then:

1. Generate the self-contained slices exactly as in the Workflow fan-out route step 1 below (`slice-batch-inputs.mjs` with absolute paths and `--language-directive`).
2. Generate EVERY batch deterministically:
   ```bash
   node <SKILL_DIR>/generate-machine-batches.mjs $PROJECT_ROOT --locale <$OUTPUT_LANGUAGE code> --all-tiers
   ```
   `--all-tiers` extends the machine-tier generator to code/script/config/infra/SQL batches: code files get file nodes plus tree-sitter function/class child nodes (same significance filter as the file-analyzer protocol) with `contains` edges; SQL mappers keep their `defines_schema`/variant edges; everything carries template summaries in the output language. The script prints `{ generated, files, ... }` — `generated` MUST equal `totalBatches`; if not, treat missing batches as a hard failure and report.
3. Skip the batch-model gate, the Workflow fan-out, and the inline dispatch loop entirely. Continue directly at the merge step (`merge-batch-graphs.py`) below.
...
2. **Batch-model gate.** Every `batches.json` entry carries a deterministic `tier` field: `code` (contains application-code files — the summaries humans actually read), `light` (non-code with real content — script/config/infra/SQL/schema definitions; template summaries won't do, but a lighter model suffices), or `machine` (pure markup/docs/csv and SQL-mapper XML — already pre-generated in step 1 with **no LLM at all**, unaffected by this gate). Call edges are resolved deterministically in Phase 3.5, and judgment-heavy single dispatches (architecture-analyzer, tour-builder, domain assignment) always use the session model regardless.

   **공통 모델 질문 규약** (map fill·screens·policy 채움과 동일): 플랫폼이 대화형 질문(AskUserQuestion 등)을 지원하면 실행 시작 시 **한 번** 묻는다. 이 스킬만 중요/덜중요 두 슬롯을 나눠 한 호출에 질문 2개로 묻는다:
   - **질문 1 — 핵심 코드(code 티어) 모델**: ① **세션 모델 (권장·기본)** — 사람이 읽는 요약문 품질 최우선 → `codeModel: "inherit"` ② sonnet — 품질·비용 균형 → `"sonnet"` ③ haiku — 최대 절감 → `"haiku"`
   - **질문 2 — 경량 배치(light 티어) 모델**: ① **sonnet (권장·기본)** — 물량 구간 절감, 스크립트/설정 요약엔 충분 → `lightModel: "sonnet"` ② 세션 모델 — 절감 없음 → `"inherit"` ③ haiku — 최대 절감 → `"haiku"`

   If the platform cannot ask interactively, don't ask — default to `codeModel: "inherit"`, `lightModel: "sonnet"` (혼합 라우팅). Then compute the light/machine batch index lists:
   ```bash
...
For each remaining batch, dispatch a subagent using the `file-analyzer` agent definition (at `agents/file-analyzer.md`). Run up to **5 subagents concurrently**. If the platform's agent dispatch supports a per-call model override (e.g. the Agent tool's `model` parameter), apply the **공통 모델 질문 규약**: if interactive questions are available, ask ONCE at the start of this dispatch loop — the same two-slot question as the Workflow route's batch-model gate (질문 1 core 티어: 세션 모델 권장·기본 / sonnet / haiku, 질문 2 light 티어: sonnet 권장·기본 / 세션 모델 / haiku). If not interactive, don't ask — default to code=세션 모델, light=sonnet. Batches whose entry has `tier: "light"` (non-code with real content — script/config/infra/SQL/schema definitions) dispatch with the chosen light model, `tier: "code"` batches with the chosen core model (no override when 세션 모델). Entries without a `tier` field (older batches.json) count as `code`. If no model override is supported, dispatch as-is without asking. Append the following additional context:
...
**Lite mode skips this phase.** If `$MODE=lite`, report `[Phase 5/7] Skipped (lite mode — no guided tour)`, use an empty `tour: []` in the Phase 6 assembly, and continue at Phase 6.
...
```json
{
  ...
  "tour": [<steps from Phase 5; [] when $MODE=lite>]
}
```
...
3. Write metadata to `$PROJECT_ROOT/.understand-anything/meta.json` (only after step 2 succeeded):
   ```json
   {
     ...
     "analysisMode": "<lite | full — the $MODE this run used; Phase 0 mode resolution reads it on the next run>"
   }
   ```
