...
import type { CensusReport, RouteEntry, RouteMethod } from '../types.js'

const HTTP_METHODS: RouteMethod[] = ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'HEAD', 'OPTIONS']
...
async function extractRouteMethods(absPath: string): Promise<RouteMethod[]> {
  let src: string
  try {
...
function collectExportedNames(root: Node, into: Set<string>): void {
  for (const child of root.namedChildren) {
    if (!child) continue
    if (child.type !== 'export_statement') continue
    const decl = child.namedChildren.filter((c): c is Node => c !== null)
    for (const d of decl) {
      if (d.type === 'function_declaration') {
        const id = d.childForFieldName('name')
        if (id) into.add(id.text)
    ...
    if (appIdx !== -1 && isAppRouterFile(parts, appIdx)) {
      ...
      if (base === 'route') {
        ...
        if (methods.length === 0) {
          ...
          for (const m of methods) out.push(makeRoute(m, apiPath, 'api', rel))
        }
        continue
      }
      continue
 ...
 */
function isAppRouterFile(parts: string[], appIdx: number): boolean {
  const fileName = parts[parts.length - 1]
  const base = stripExt(fileName)
  const ext = fileName.slice(fileName.lastIndexOf('.'))
  if (!PAGE_EXTS.includes(ext)) return false
  if (base !== 'page' && base !== 'route') return false
