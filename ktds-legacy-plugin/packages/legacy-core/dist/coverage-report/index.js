...
export const CoverageReportSchema = z.object({
    ...
    interfaces: z
        ...
        byProtocol: z.array(z.object({ protocol: z.string(), count: z.number().int().nonnegative() })),
    ...
    langSupport: z
        ...
        byLang: z.array(z.object({
            lang: z.string(),
            files: z.number().int().nonnegative(),
            best: z.enum(['full', 'partial', 'none']),
            core: z.enum(['full', 'partial', 'none']),
            capabilities: z.array(z.object({ key: z.string(), tier: z.enum(['full', 'partial', 'none']) })),
        })),
    })
        .optional(),
});
/** 스캔 산출물에서 통합 커버리지 리포트를 결정론으로 조립(AC-30). */
export function buildCoverageReport(inputs) {
    ...
    const jpa = {
        ...
        tierCQueries: inputs.jpaModel?.repositories.reduce((n, r) => n + r.queries.filter((q) => q.native).length, 0) ?? 0,
    };
