...
import { RoutesReportSchema, type CensusReport, type RoutesReport } from './types.js'
import {
  CENSUS_FILENAME,
  ROUTES_FILENAME,
  gitCommitDate,
...
import { readDbSchema, type DbSchemaModel, type DbTable } from '../db-schema/index.js'
...
export interface MinimalKg {
  ...
  ktdsStructure: { generatedFromCommit: string; minimal: true }
}
...
function nodeTypeForLang(lang: string): MinimalKgNodeType {
  if (CONFIG_LANGS.has(lang)) return 'config'
  if (SCHEMA_LANGS.has(lang)) return 'schema'
  if (DOCUMENT_LANGS.has(lang)) return 'document'
  return 'file'
}
 ...
 */
export const SECRET_CARRIER_NAME_PATTERNS: RegExp[] = [/keystore/i]
...
export function isSecretCarrierPath(relPath: string): boolean {
  const base = relPath.slice(relPath.lastIndexOf('/') + 1)

  // 최우선 — ssh 개인키 프리픽스는 확장자 무관(위 주석 참조).
  if (SSH_PRIVATE_KEY_PREFIX_RE.test(base)) return !/\.pub$/i.test(base)

  const segments = base.split('.')
  const lastExt = segments.length > 1 ? segments[segments.length - 1].toLowerCase() : ''

  // 템플릿/샘플 접미사 — 공유 안전 사본은 제외 대상에서 뺀다.
  if (TEMPLATE_SUFFIX_SET.has(lastExt)) return false

  // 최종 확장자 지배 — 소스/문서로 확정되면 이 아래(세그먼트 스캔·keystore 패턴)는
  // 전부 건너뛴다(오탐 방지, 위 주석 참조).
  if (SOURCE_DOC_EXT_SET.has(lastExt)) return false

  if (SECRET_CARRIER_NAME_PATTERNS.some((re) => re.test(base))) return true
  ...
  for (let i = 1; i < segments.length; i++) {
    if (SECRET_CARRIER_EXT_SET.has(segments[i].toLowerCase())) return true
  }
  return false
}

/** 경로 세그먼트에 "test" 가 있으면 tags=["test"](최소 결정론 신호), 아니면 []. */
function tagsForPath(relPath: string): string[] {
  return relPath.split('/').includes('test') ? ['test'] : []
}

function fileNodeFor(f: CensusReport['files'][number]): MinimalKgNode {
  return {
    id: `file:${f.relPath}`,
...
function tableLineRange(t: DbTable): [number, number] {
  let end = t.line
  for (const c of t.columns) if (c.line > end) end = c.line
  for (const fk of t.foreignKeys) if (fk.line > end) end = fk.line
  for (const c of t.checks) if (c.line > end) end = c.line
  for (const ix of t.indexes) if (ix.line > end) end = ix.line
  return [t.line, end]
}

function tableNodeFor(t: DbTable): MinimalKgNode {
  return {
    id: `table:${t.name}`,
    type: 'table',
    name: t.name,
 ...
 */
export function buildMinimalKg(inputs: MinimalKgBuildInputs): MinimalKg {
  ...
  const tablesSorted = [...inputs.dbSchema.tables].sort(
    (a, b) => cmp(a.name, b.name) || cmp(a.relPath, b.relPath),
  )
  const seenTableNames = new Set<string>()
  const tableNodes: MinimalKgNode[] = []
  const edges: MinimalKgEdge[] = []
  for (const t of tablesSorted) {
    if (seenTableNames.has(t.name)) continue
 ...
 */
export function loadMinimalKgInputs(
  projectRoot: string,
): Omit<MinimalKgBuildInputs, 'analyzedAt' | 'projectName'> {
  const census = readMapArtifact(projectRoot, CENSUS_FILENAME, CensusReportSchema)
  if (census === null) {
    throw new MinimalKgInputMissingError(
      `${CENSUS_FILENAME} 없음 — 먼저 /understand-map scan을 실행하세요(.spec/map/ 산출물 필요)`,
    )
  }
  const dbSchema = readDbSchema(projectRoot)
  if (dbSchema === null) {
    throw new MinimalKgInputMissingError(
      'db-schema.json 없음 — 먼저 /understand-map scan을 실행하세요(.spec/map/ 산출물 필요)',
    )
  }
...
function hasKtdsStructureMarker(filePath: string): boolean {
  let raw: unknown
  try {
  ...
  return typeof raw === 'object' && raw !== null && 'ktdsStructure' in (raw as Record<string, unknown>)
}
 ...
 */
export function writeMinimalKg(
  projectRoot: string,
  kg: MinimalKg,
  options: { overwriteKg?: boolean } = {},
  ...
  if (existsSync(filePath) && !options.overwriteKg && !hasKtdsStructureMarker(filePath)) {
    return {
      ...
      path: filePath,
      message:
        '기존 knowledge-graph.json 을 보존합니다(/understand 산출로 보이며 ktdsStructure 마커가 없음). ' +
 ...
 */
export function emitMinimalKg(
  projectRoot: string,
  options: { overwriteKg?: boolean; analyzedAt?: string; projectName?: string } = {},
): MinimalKgWriteResult {
