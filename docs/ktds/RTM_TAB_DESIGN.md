...
> **탭 목록 갱신(W2, 2026-07-16)**: 위 "뷰 2개"에 더해 추적표 헤더에 **"요청 세션" 탭**이 신설됐다
> (좌 270px 세션 원장 + 우 **6단계** 스테퍼·산출물 — `RTM_INTAKE_WORKSPACE_DESIGN.md` §2.2,
> `RTM_STEP_FLOW_DESIGN.md` §2). 이 탭은 위 "전치"
> 규정의 **엄밀한 예외**다 — 세션(`rtm-intake/<sid>/session.json`)은 `rtm.json`의 전치가 아니라
> 인테이크 **진행상태 데이터**이기 때문. 탭 라벨을 동사("새 요청")가 아닌 **대상 명사**("요청 세션")로
> 두어 렌즈 관례에 최대한 맞춰 마찰을 완화한다(완전 해소는 아님 — `RTM_INTAKE_WORKSPACE_DESIGN.md`
> §2.1·C4 참조).
...
1. 입력: 자연어 + **현재 도메인/기능 인벤토리**(컨텍스트로 주입 → 기존 입도와 일관되게 분해).
   **구현됨(P5, 2026-07-16)** — 단 이 문서가 그리는 직접 주입이 아니라 `rtm-intake.mjs intake-input`의
   **6축 근거 번들 경유**로 인벤토리가 딸려 들어간다(`RTM_IMPACT_GATE_DESIGN.md` §6/§9 P3~P5).
2. `claude -p` 가 판정:
   ...
   - **기존 도메인 수정?**(EX2 결제) → 도메인 인벤토리에서 매칭 → 그 도메인 **안에** 행 추가 +
     ...
     **구현됨(P6, 2026-07-16)** — 단 ① 안이 아니라 **독립 단계 ②영향분석**이다
     (`rtm-intake.mjs code-impact` — `changeset.modified` → 시드 결정론 조인 → 도달성.
     6단계 승격 경위는 `RTM_IMPACT_GATE_DESIGN.md` §6.5). ①식별은 근거 번들의 6축 요약을 인용해
     분해까지만 하고(P5), 코드영향은 ②가 엔진으로 산출한다.
...
| REQ → 기능 N개 changeset / 고아 판정 | `/understand-impact` 엔진 — **미배선**(P6 진행 중, 2026-07-16 기준) |
