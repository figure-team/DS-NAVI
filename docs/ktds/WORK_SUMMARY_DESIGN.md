...
| P7-f 다주 추이(W6-b) | ✅ | de877cf~41614a0 | §13 — previous 집계·증감 파생·코드 리뷰 4건 반영(하단 disposition) |
...
## 13. 다주 추이 (W6-b, P7-f) — 직전 기간 대비 증감

P7 리뷰가 "주간보고 최대 가치 축"으로 꼽은 갭. 사용자 결정으로 P8 앞에 선행(2026-07-05).

- **직전 기간 정의(결정론)**: 현재 윈도와 **동일 길이·인접** —
  - weeks N: 현재 (anchor−N주, anchor] 의 직전 = (anchor−2N주, anchor−N주] (동일 반개구간 방향)
  - month: 직전 달력 월 [전월 1일, 당월 1일) UTC
  - range: 시각 축 없음 → 추이 없음(null, [미확인] — 진척과 동일 원칙)
- **수집 확장**: 스크립트의 `--since` 바운드를 두 윈도를 덮게 확장(weeks: 2N×7+1일,
  month: 전월 1일 −1일). 수집기는 무변경 — build 단계가 직전 윈도를 한 번 더 필터.
- **산출물**: `previous: null | { fromIso, toIso, totals(동일 구조·생성물 분리 포함),
  rtmProgress|null, docProgress|null }` 필드 추가(additive — schemaVersion 유지, 증감은
  저장하지 않고 문서 빌더가 파생 계산: 원천 중복 금지).
- **원장 추이의 한계(정직)**: RTM/문서 진척 추이도 **현재 원장 상태**에서 두 윈도를
  각각 집계한 것 — §3.4 재현 경계가 동일하게 적용됨을 §2 기준에 명기.
- **문서**: §1 하이라이트에 "직전 기간 대비" 행(커밋 a→b(±d)·라인·RTM 전환), §2 에
  추이 산정 기준 행. 모듈 단위 추이는 백로그(표 폭주 방지).
- **검증**: 직전 윈도 경계(현재 하한 = 직전 상한, 이중 계상 0)·range/git 불가 null·
  byte 결정론. 실측 = 이 레포 weeks 1(직전 주 = P1~P5 기간, 데이터 풍부).

### §13 실측·리뷰 반영 (2026-07-05, 41614a0)

- 실측(이 레포 weeks 1): 직전 주(재구축·화면설계서 기간) 커밋 56건 → 이번 주 91건(+35),
  실적 라인 107,823→33,541(−74,282) — 사람이 읽는 그대로의 추이 문장이 §1 에 실림.
  경계 커밋(현재 하한 정각)은 직전 윈도에만 귀속(이중 계상 0, 테스트 단언). 재실행
  md5 동일. jpetstore(2주): 직전 0건→12건(+12) 정상. range 모드 previous null.
- 코드 리뷰(code-reviewer, REQUEST CHANGES → 전건 반영): **T1(H)** 구버전 산출물
  (previous 키 부재, raw JSON 로드)이 trendRows 를 크래시시켜 뒤 문서 5종 생성까지
  중단 — `!p` 가드 + understand-docs 의 WorkSummaryReportSchema.safeParse degrade
  (형식 불일치 시 현황 행 + 재수집 안내, **jpetstore 실물 재현으로 검증**: 14종 전부
  생성 유지). T2(L) 월간 추이 여는 괄호 `[` 모드별화. T3(L) 직전 달 rawArg 롤오버
  (2026-01 → 2025-12, 창·라벨 일치). T4(L) 테스트 공백 — 1월 롤오버·previous 부재·
  월간 괄호 3케이스 추가. 916 green.
- 설계 비평 레인은 별도 실행하지 않음 — W6-b 자체가 P7 비평(§12 갭 "다주 추이 = 주간
  보고 최대 가치 축")의 이행이라 설계 근거가 이미 비평 산출물임을 명기.
