...
import { z } from 'zod'
import { CITATION_STATUS, type CitationStatus } from '../types.js'
...
function verifyCitation(
  ...
  if (abs !== rootAbs && !abs.startsWith(rootAbs + sep)) return 'path-escape'
...
export function verifyImpactClaims(
  projectRoot: string,
...
export function verifyOneCitation(projectRoot: string, citation: ImpactCitation): CitationStatus {
 ...
 */
export function verifyAnchorExists(
  projectRoot: string,
  ...
  try {
    const real = realpathSync(abs)
    const realRoot = realpathSync(rootAbs)
    if (real !== realRoot && !real.startsWith(realRoot + sep)) return 'path-escape'
    lines = readFileSync(real, 'utf8').split('\n')
  } catch {
    return 'no-file'
  }
  // 전부 공백인 파일은 닻을 걸 실체가 없다 — 빈 파일에 line 1 'ok' 로 위양성 grounding 방지.
  if (lines.every((l) => l.trim() === '')) return 'no-file'
  if (anchor.line < 1 || anchor.line > lines.length) return 'line-out-of-range'
  return 'ok'
}
