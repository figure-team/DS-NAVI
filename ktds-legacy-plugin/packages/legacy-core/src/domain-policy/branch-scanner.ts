...
import { parseSource, startLine, type LangId } from '../domain-map/tree-sitter.js'
...
function langOf(relPath: string): LangId {
...
function nameOrIdent(node: Node): string | null {
  return nameField(node) ?? node.namedChildren.find((c): c is Node => c != null && c.type === 'identifier')?.text ?? null
}
 ...
 */
async function extractBranchesJava(relPath: string, src: string): Promise<BranchSignal[]> {
 ...
 */
async function extractBranchesKotlin(relPath: string, src: string): Promise<BranchSignal[]> {
  ...
  const walk = (node: Node, className: string | null, methodName: string | null): void => {
    ...
    if (node.type === 'if_expression') {
      ...
      const subj = node.namedChildren.find((c): c is Node => c != null && c.type === 'when_subject')
      out.push({
        relPath,
        line: startLine(node),
        className: cls,
...
async function extractEnumsJava(relPath: string, src: string): Promise<EnumFact[]> {
  const root = await parseSource('java', src)
  const out: EnumFact[] = []
...
async function extractEnumsKotlin(relPath: string, src: string): Promise<EnumFact[]> {
  const root = await parseSource('kotlin', src)
  const out: EnumFact[] = []
  const isEnumClass = (node: Node): boolean => {
    const mods = node.namedChildren.find((c): c is Node => c != null && c.type === 'modifiers')
    return mods != null && /\benum\b/.test(mods.text)
  }
  const walk = (node: Node): void => {
    if (node.type === 'class_declaration' && isEnumClass(node)) {
      const name = nameOrIdent(node) ?? ''
      const body = node.namedChildren.find((c): c is Node => c != null && c.type === 'enum_class_body')
      const constants: string[] = []
      if (body) {
        for (const c of body.namedChildren) {
