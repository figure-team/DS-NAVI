...
/** 경로 후보 1개(rawPath + notes). Java판 PathCandidate 와 동형. */
 ...
 */
function resolveKtPathExpr(
  node: Node,
  ctx: SpringContext,
): { value: string | null; unresolvedRef?: string; note?: string } {
  if (node.type === 'string_literal') {
    const s = ktStringContent(node)
    if (s !== null) return { value: s }
    return { value: null, unresolvedRef: node.text, note: `unresolved-constant:${node.text}` }
  }
  if (node.type === 'parenthesized_expression') {
    const inner = node.namedChildren.filter((c): c is Node => c !== null)[0]
    if (inner) return resolveKtPathExpr(inner, ctx)
    return { value: null, unresolvedRef: node.text, note: `unresolved-constant:${node.text}` }
  }
...
function collectKtPathExprs(node: Node, ctx: SpringContext, out: PathCandidate[]): void {
  if (node.type === 'collection_literal') {
    for (const child of node.namedChildren) {
      if (child) collectKtPathExprs(child, ctx, out)
    }
    return
  }
...
function collectKtMethods(node: Node, out: RouteMethod[]): void {
  if (node.type === 'collection_literal') {
    for (const child of node.namedChildren) {
      if (child) collectKtMethods(child, out)
    }
    return
  }
...
function findKotlinClassDeclarations(root: Node): Node[] {
  const out: Node[] = []
...
function ktReturnTypeText(method: Node): string {
  const skip = new Set([
    'modifiers',
    'identifier',
  ...
  for (const c of method.namedChildren) {
    if (c && !skip.has(c.type)) return c.text
  }
  return ''
}
 ...
 */
export function extractSpringKotlinRoutes(root: Node, filePath: string, ctx: SpringContext): RouteEntry[] {
 ...
 */
export function collectKotlinConstants(root: Node, into: Map<string, string>): void {
  walkForKtConstants(root, null, into)
}

function walkForKtConstants(node: Node, enclosingClass: string | null, into: Map<string, string>): void {
  for (const c of node.namedChildren) {
    ...
    if (c.type === 'class_declaration' || c.type === 'object_declaration') {
      ...
      if (clsBody) walkForKtConstants(clsBody, nm, into)
      continue
    }
    if (c.type === 'companion_object') {
      const coBody = ktChild(c, 'class_body')
      if (coBody) walkForKtConstants(coBody, enclosingClass, into)
      continue
    }
    if (c.type === 'property_declaration') {
      ...
      if (isConst) {
        ...
        const named = c.namedChildren.filter((x): x is Node => x !== null)
        const valueNode = named[named.length - 1]
        const value =
          valueNode && valueNode !== varDecl && valueNode !== mods ? ktStringContent(valueNode) : null
        if (nameId && value !== null) {
          if (enclosingClass) into.set(`${enclosingClass}.${nameId.text}`, value)
