...
import type { RtmDiagnostic, RtmModel, RtmRequirement, RtmTraceCell } from './types.js'
 ...
 */
function assertsConfirmed(text: string, confidence: Confidence): boolean {
 ...
 */
export function checkCellGrounding(model: RtmModel, inventory?: RtmCellInventory): RtmDiagnostic[] {
  const tables = inventory?.tables ? new Set(inventory.tables) : null
  if (!tables) return [] // 축 미주입 → 대조 생략(하위호환)

  const out: RtmDiagnostic[] = []
  for (const f of model.functions) {
    const cells: Array<[string, RtmTraceCell]> = [
