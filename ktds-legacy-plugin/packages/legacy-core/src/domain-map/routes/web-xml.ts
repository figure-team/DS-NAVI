...
function elementText(body: string, tag: string): string | null {
...
function elementTexts(body: string, tag: string): string[] {
  const out: string[] = []
...
export function extractWebXmlRoutes(rawText: string, filePath: string): RouteEntry[] {
  const text = stripXmlComments(rawText)
  const out: RouteEntry[] = []

  // 1) <servlet> 정의 수집: servlet-name -> { className, jspFile }.
  const defs = new Map<string, ServletDef>()
 ...
 */
export function extractWebXmlRoutesFromCensus(
  projectRoot: string,
  census: CensusReport,
): RouteEntry[] {
  const out: RouteEntry[] = []
  for (const file of census.files) {
    if (!file.relPath.endsWith('/web.xml') && file.relPath !== 'web.xml') continue
    let text: string
    try {
