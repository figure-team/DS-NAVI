 ...
 */
export function isDomainIneligibleRoot(relPath: string): boolean {
  const p = relPath.toLowerCase()
  if (/(^|\/)src\/(test|it)\//.test(p)) return true
  if (/(^|\/)__tests?__\//.test(p)) return true
  if (/\.(jsp|jspx|html?|css)$/.test(p)) return true
  return false
}
