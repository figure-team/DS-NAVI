...
function grammarFor(relPath: string): 'typescript' | 'tsx' {
...
function literalPathValue(node: Node): string | null {
  if (node.type === 'string') {
    const frag = childrenOfType(node, 'string_fragment')[0]
    return frag ? frag.text : ''
  }
  if (node.type === 'template_string') {
    if (childrenOfType(node, 'template_substitution').length > 0) return null // 보간 포함 — 정적 해소 불가.
    const frag = childrenOfType(node, 'string_fragment')[0]
    return frag ? frag.text : ''
  }
  return null
 ...
 */
export function collectRelativeImportSpecs(root: Node): RawImportRef[] {
  const out: RawImportRef[] = []
 ...
 */
export function resolveRelativeSpec(
  fromRelPath: string,
  spec: string,
 ...
 */
export async function extractTsImportEdges(
  projectRoot: string,
  census: CensusReport,
): Promise<EdgeRecord[]> {
  ...
  for (const f of sorted) {
    ...
    for (const ref of collectRelativeImportSpecs(root)) {
      ...
      if (!target || target === f.relPath) continue // 자기참조 제외(edges.ts 와 동일 규약).
