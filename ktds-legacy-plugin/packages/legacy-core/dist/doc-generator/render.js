...
function evidenceSuffix(claim) {
    if (claim.evidence.length === 0)
        return '';
    const anchors = claim.evidence.map((e) => e.line === null ? `\`${e.file}\`` : `\`${e.file}:${e.line}\``);
...
function renderClaimsBlock(claims) {
    if (claims.length === 0)
 ...
 * 밖(§3.3)이며 골든 비대상이다.
 */
export function renderMarkdown(doc, meta) {
    const lines = [
 ...
 */
export function renderSkeleton(doc) {
    const lines = [];
    for (const s of doc.sections) {
        lines.push(`## ${s.heading}`, '');
        lines.push(...renderSectionBody(s), '');
