...
function parseFrontmatter(md) {
    const m = /^\uFEFF?---\r?\n([\s\S]*?)\r?\n---/.exec(md);
    if (!m)
        throw new Error('문서 템플릿에 frontmatter(--- ... ---)가 없습니다');
    const out = {};
 ...
 */
export function parseDocTemplate(md) {
 ...
 */
export function applyDocTemplate(doc, tpl) {
    ...
    const withColumns = (data, ts) => {
        const out = { ...data, key: ts.key };
    ...
    for (const ts of tpl.sections) {
        ...
        if (matches.length === 0) {
            ...
            for (const m of matches)
                sections.push(withColumns(m, ts));
        }
    }
