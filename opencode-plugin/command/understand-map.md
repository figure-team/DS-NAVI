...
> 🧩 **opencode 런타임:** 번들 스크립트는 `$ATLAS_PLUGIN_ROOT/scripts/understand-map.mjs` 로 호출한다(이 변수는 atlas 플러그인이 셸에 주입한다). `<projectRoot>` 는 인자 `$ARGUMENTS` 의 첫 토큰(없으면 현재 작업 디렉터리).
...
```
node $ATLAS_PLUGIN_ROOT/scripts/understand-map.mjs <projectRoot> scan
```
...
```
node $ATLAS_PLUGIN_ROOT/scripts/understand-map.mjs <projectRoot> plan
```
...
node $ATLAS_PLUGIN_ROOT/scripts/understand-map.mjs <projectRoot> confirm

# 확정 실행(후보를 그대로 수용해 확정 플랜 기록)
node $ATLAS_PLUGIN_ROOT/scripts/understand-map.mjs <projectRoot> confirm --auto-approve --by <담당자>
```
...
```
node $ATLAS_PLUGIN_ROOT/scripts/understand-map.mjs <projectRoot> map
```
...
```
node $ATLAS_PLUGIN_ROOT/scripts/understand-map.mjs <projectRoot> bundle
```
...
## 6) 호스트 채움 — fill 작성 (인용 의무)

번들과 emit 사이에서 **호스트(opencode 에이전트)가 도메인당 1회** 채움을 수행한다:
...
```
node $ATLAS_PLUGIN_ROOT/scripts/understand-map.mjs <projectRoot> emit
```
