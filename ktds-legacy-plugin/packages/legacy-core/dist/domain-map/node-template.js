...
function roleSection(promptHint) {
 ...
 */
export const DEFAULT_NODE_DETAIL_TEMPLATE = {
    version: 2,
    byLayer: {
        api: [
 ...
 * `## ` 앞의 제목(`#`)/설명(`>`) 프로즈는 무시한다.
 */
export function parseLayerSections(md) {
    const sections = [];
    let cur = null;
    const flush = () => {
        if (cur) {
            ...
            if (!promptHint)
                throw new Error(`섹션 '${cur.id}' 의 promptHint(본문)가 비어 있습니다`);
 ...
 */
export function parseNodeDetailTemplate(filesByLayer) {
    const byLayer = {};
    for (const [layer, md] of Object.entries(filesByLayer)) {
        if (md == null)
            continue;
        byLayer[layer] = parseLayerSections(md);
    }
 ...
 */
export function sectionsForLayer(template, layer) {
    const key = layer && layer in template.byLayer ? layer : 'unknown';
