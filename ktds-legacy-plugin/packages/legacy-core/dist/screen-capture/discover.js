/** 정적 자산 확장자 — 크롤 방문 제외. */
const ASSET_EXT_RE = /\.(css|js|mjs|png|jpe?g|gif|svg|ico|webp|woff2?|ttf|eot|map|pdf|zip|gz)(\?|$)/i;
/**
...
export function relativePath(u, contextPath) {
    let path = u.pathname;
 ...
 */
export function screenKey(u, contextPath) {
 ...
 */
export function screenIdFor(u, contextPath) {
...
export function capturePathFor(screenId) {
    return `screens/${slugify(screenId.replace(/^screen:/, ''))}.png`;
}
/** 크롤 방문 정책 — 자산/제외 정규식 필터(횟수 상한은 러너의 maxPages 가 담당). */
export function shouldVisit(u, exclude) {
    const full = u.pathname + u.search;
    if (ASSET_EXT_RE.test(u.pathname))
        return false;
    for (const pattern of exclude) {
        try {
            if (new RegExp(pattern).test(full))
                return false;
        }
        catch {
...
const JSP_INCLUDE_RE = /<%@\s*include\s+file\s*=\s*"([^"]+)"|<jsp:include\s+[^>]*page\s*=\s*"([^"]+)"/g;
/** '.'/'..' 세그먼트 정규화(선행 '/' 제거). */
...
function resolveIncludeTarget(fromPath, target, known) {
 ...
 */
export function detectFragments(jsps) {
 ...
 */
export function reconcileJsps(graphJsps, screens, fragments) {
    const mapped = new Set(screens.map((s) => s.jspFile).filter((f) => f !== null));
