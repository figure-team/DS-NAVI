...
function ktAnnoArgs(anno: KtAnnotation): { byKey: Map<string, string>; positional: string | null } {
  const byKey = new Map<string, string>()
  let positional: string | null = null
  for (const a of anno.args) {
    const value = ktStringContent(a.node) ?? a.node.text.trim()
    if (a.name) byKey.set(a.name, value)
    else if (positional === null) positional = value
  }
  return { byKey, positional }
}

/** nullable_type 을 벗겨 user_type(또는 그 외 타입 노드)을 노출. */
function ktUnwrapNullable(node: Node | null): Node | null {
  if (!node) return null
  if (node.type === 'nullable_type') {
    return ktUnwrapNullable(node.namedChildren.filter((c): c is Node => c !== null)[0] ?? null)
  }
  return node
}

/** 타입 노드에서 원소 타입(제네릭이면 첫 type_projection, 예: List<Order> → Order). Java elementType 대응. */
function ktElementType(typeNode: Node | null): string | null {
  ...
  if (t.type === 'user_type') {
    ...
    if (typeArgs) {
      ...
      const inner = proj ? ktUnwrapNullable(proj.namedChildren.filter((c): c is Node => c !== null)[0] ?? null) : null
...
function ktPrimaryConstructorParams(decl: Node): Node[] {
...
function ktParamNameType(param: Node): { name: string | null; type: Node | null } {
  const named = param.namedChildren.filter((c): c is Node => c !== null)
  const idIdx = named.findIndex((c) => c.type === 'identifier')
  if (idIdx < 0) return { name: null, type: null }
...
function ktPropNameType(prop: Node): { name: string | null; type: Node | null } {
  const vd = ktChild(prop, 'variable_declaration')
  if (!vd) return { name: null, type: null }
  const named = vd.namedChildren.filter((c): c is Node => c !== null)
  const id = named.find((c) => c.type === 'identifier')
  if (!id) return { name: null, type: null }
  return { name: id.text, type: named.find((c) => c !== id) ?? null }
}

function ktFindAnno(annos: KtAnnotation[], name: string): KtAnnotation | undefined {
  return annos.find((a) => a.name === name)
}

/** class_declaration(entity 후보, class_parameter + property_declaration 양쪽 필드) → JpaEntity. */
function extractKotlinEntity(decl: Node, relPath: string): JpaEntity | null {
  const declAnnos = collectDeclAnnotations(decl)
  if (!ktFindAnno(declAnnos, 'Entity')) return null
  ...
  const fieldCandidates: Array<{ node: Node; fname: string | null; ftype: Node | null; fannos: KtAnnotation[] }> = []
  ...
  for (const { node: field, fname, ftype, fannos } of fieldCandidates) {
    if (!fname) continue
    if (ktFindAnno(fannos, 'Id')) idField = fname
    if (ktFindAnno(fannos, 'Transient')) continue
...
function ktSpringDataBase(decl: Node): { base: string; entity: string | null; id: string | null } | null {
  ...
  for (const spec of ktChildren(ds, 'delegation_specifier')) {
    ...
    if (!baseName || !SPRING_DATA_BASES.has(baseName)) continue
    const typeArgs = ktChild(ut, 'type_arguments')
    const args: string[] = []
    if (typeArgs) {
      for (const proj of ktChildren(typeArgs, 'type_projection')) {
        const inner = proj.namedChildren.filter((c): c is Node => c !== null)[0] ?? null
...
function extractKotlinRepository(
  decl: Node,
  relPath: string,
): { repo: JpaRepository; unresolved: Array<{ ref: string; reason: string }> } | null {
...
function collectKtClassDecls(root: Node): Node[] {
  const out: Node[] = []
  const walk = (n: Node): void => {
    for (const c of n.namedChildren) {
      if (!c) continue
...
export async function extractJpaFromKotlinSource(
  source: string,
  relPath: string,
...
/** W8 캐시 팩트 — java/kotlin 파일 1개의 JPA 기여분(내용의 순수 함수, 실패 메시지 포함). */
