...
export function ktChild(node: Node, type: string): Node | null {
...
export function ktChildren(node: Node, ...types: string[]): Node[] {
...
export function ktDeclKind(decl: Node): KtDeclKind {
  if (decl.type === 'object_declaration') return 'object'
  for (const c of decl.children) {
    if (!c) continue
    if (c.type === 'interface') return 'interface'
    if (c.type === 'class') break
  }
  // enum 은 `enum class` — modifiers > class_modifier 텍스트 또는 enum_class_body 로 판정.
  if (ktChild(decl, 'enum_class_body')) return 'enum'
  const mods = ktChild(decl, 'modifiers')
  if (mods && ktChildren(mods, 'class_modifier').some((m) => m.text === 'enum')) return 'enum'
  return 'class'
}
 ...
 */
export function ktTypeOuterName(node: Node | null): string | null {
  if (!node) return null
  switch (node.type) {
    case 'identifier':
    case 'type_identifier':
      return node.text
    case 'nullable_type':
      return ktTypeOuterName(node.namedChildren.filter((c): c is Node => c !== null)[0] ?? null)
    case 'user_type': {
...
export function ktAnnotationName(anno: Node): string | null {
...
function argsFromValueArguments(va: Node | null): KtAnnoArg[] {
  if (!va) return []
  const out: KtAnnoArg[] = []
...
function argsFromParenExpr(paren: Node): KtAnnoArg[] {
  const out: KtAnnoArg[] = []
...
function annoFact(anno: Node): KtAnnotation | null {
...
function annotationsFromChain(annotatedExpr: Node): KtAnnotation[] {
  const out: KtAnnotation[] = []
  let cursor: Node | null = annotatedExpr
  let pendingParen: Node | null = null
  while (cursor) {
  ...
  if (pendingParen && out.length > 0) {
    ...
    if (last.args.length === 0) last.args = argsFromParenExpr(pendingParen)
  }
  return out
}
 ...
 */
export function collectDeclAnnotations(decl: Node): KtAnnotation[] {
  const out: KtAnnotation[] = []
  // 1) 분리형 치유 — 직전 실질 형제(주석 건너뜀)가 annotated_expression 이면 재결합.
  if (HEALABLE_DECLS.has(decl.type) && decl.parent) {
    const sibs = decl.parent.namedChildren.filter((c): c is Node => c !== null)
    const i = sibs.findIndex((s) => s.id === decl.id)
    let j = i - 1
    while (j >= 0 && /comment/.test(sibs[j].type)) j--
...
export function ktStringContent(node: Node | null): string | null {
  if (!node) return null
  ...
  if (node.type === 'parenthesized_expression') {
    return ktStringContent(node.namedChildren.filter((c): c is Node => c !== null)[0] ?? null)
  }
  return null
}

/** package_header 의 FQN. */
export function ktPackageName(root: Node): string | null {
...
export function ktImports(root: Node): string[] {
  const out: string[] = []
