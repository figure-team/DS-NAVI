...
describe('analyzedAt 결정론 센티널(적대 리뷰 C1) — now() 폴백 제거 확인', () => {
  ...
  it('gitCommit 이 저장소에 실재 — analyzedAt 은 커밋 커미터 시각(UTC 정규화)', () => {
    // 성공 분기(수정의 핵심 가치: analyzedAt = 실제 커미터 시각)를 검증한다 —
    // 기존 두 테스트는 센티널 폴백만 덮었다(적대 리뷰 P3). 비-UTC 오프셋을 줘
    // UTC 정규화(적대 리뷰 결정론 checklist #6)도 함께 확인한다.
    execFileSync('git', ['init', '-q'], { cwd: root })
    execFileSync(
      'git',
      ['-c', 'user.email=t@t.com', '-c', 'user.name=t', 'commit', '-q', '--allow-empty', '-m', 'init'],
      { cwd: root, env: { ...process.env, GIT_COMMITTER_DATE: '2020-01-02T03:04:05+09:00', GIT_AUTHOR_DATE: '2020-01-02T03:04:05+09:00' } },
    )
    const sha = execFileSync('git', ['rev-parse', 'HEAD'], { cwd: root }).toString().trim()

    // +09:00 03:04:05 → UTC 18:04:05 전일.
    expect(resolveAnalyzedAt(root, sha)).toBe('2020-01-01T18:04:05.000Z')
    // 방어: null·부재 커밋은 센티널(기존 분기 재확인).
    expect(resolveAnalyzedAt(root, null)).toBe(ANALYZED_AT_SENTINEL)
  })
})
