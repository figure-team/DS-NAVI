...
export function specDocsDir(projectRoot) {
    return join(projectRoot, '.spec', 'docs');
}
/** `.spec/docs/<docId>.state.json` 파일 경로. */
export function docStatePath(projectRoot, docId) {
 ...
 */
export function writeDocState(projectRoot, docId, state) {
 ...
 */
export function readDocState(projectRoot, docId) {
    const file = docStatePath(projectRoot, docId);
    let raw;
    try {
