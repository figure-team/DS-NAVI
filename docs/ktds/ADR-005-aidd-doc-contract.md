...
- **스코프 구분 (2026-06-15 사용자 지시):** 근시일 = **공통 계약 + 프로파일 R(`/understand-docs` 참조 샤드) + 방법론 모듈 시스템(Layer 1·2, D1~D4, D7, §2-A)**. **보류 = AIDD 위임구현 기능 = 프로파일 W(변경 스토리) + 위임구현 게이트(D5) + 출처 기반 근거 게이트(D6)** — 신설 기능 착수 시점에 진행. 본 ADR은 보류분의 설계 기록을 미리 확정해 둔다(통일 계약이 R/W를 함께 규정해야 일관되므로).
