...
export function detectStartCommand(build: BuildSignals): {
  command: string[] | null
  source: string | null
} {
  if (build.pomXml) {
...
function normalizeContextPath(contextPath: string | null): string {
 ...
 */
export function scaffoldScreensConfig(input: ScaffoldInput): {
  ...
  if (command) {
    ...
    notes.push(
      'startCommand 미감지 — 앱 기동 명령을 직접 채우거나, 앱을 띄워둔 채 capture 를 실행하세요(README/CI 워크플로에 실행 방법이 있는 경우가 많습니다).',
    )
  }
  notes.push(
...
function readGradle(projectRoot: string): string | null {
 ...
 */
export function scaffoldScreensConfigOnDisk(
  projectRoot: string,
  opts: { force?: boolean } = {},
): { configPath: string; summary: ScaffoldSummary } {
  ...
  if (!existsSync(routesPath)) {
    throw new Error(
      `${routesPath} 이 없습니다 — /understand-map 스캔을 먼저 실행하세요(초안 재료: 라우트 census).`,
    )
  }
  const report = JSON.parse(readFileSync(routesPath, 'utf8')) as {
    routes?: CensusRoute[]
    contextPath?: string | null
  }
