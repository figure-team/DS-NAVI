...
## 2. 확정 문서 세트 (근거 가능 6종, 사용자 승인 2026-06-20)
| docId | 문서 | 근거원 | 빌더 현황 |
|---|---|---|---|
...
범위 밖(근거 약함): 요구사항정의서·시험케이스·데이터전환 등.

## 3. 단계 (stop-per-phase)
- **D1 — 기본 템플릿(선행). ✅ 완료(미커밋→커밋 예정).** `templates/doc/` 에 6종 + `_README.md`
...
- **D2 — 템플릿 런타임 로드 + 생성 연결.** 엔진에 순수 파서 `parseDocTemplate`(node-template.ts
  ...
  06_program-list 빌더 신규. 게이트: 골든 스냅샷(기본 템플릿=현출력) 불변 + 신규 doc 스냅샷.
