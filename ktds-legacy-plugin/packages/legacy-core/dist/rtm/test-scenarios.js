 ...
 */
export function scenarioId(fnId, suffix) {
    const h8 = createHash('sha256').update(fnId).digest('hex').slice(0, 8);
    return `TS-${h8}-${suffix}`;
}
/** 한 기능 행의 시나리오 3종+ 생성. */
function scenariosOf(f) {
    const out = [];
    ...
    const exceptions = [...f.rules.filter((r) => r.kind === 'exception')].sort((a, b) => cmp(a.acId, b.acId) || cmp(a.reqId, b.reqId));
...
export function buildTestScenarios(model) {
 ...
 */
export function attachTestScenarios(model) {
    const withTs = { ...model, testScenarios: buildTestScenarios(model) };
