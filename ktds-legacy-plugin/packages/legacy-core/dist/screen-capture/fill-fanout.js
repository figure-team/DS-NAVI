...
const CONFIRMED_CONFIDENCES = new Set(['CONFIRMED', 'CONFIRMED_AI']);
// ──────────────────────────────────────────────────────────────────────────
// 스키마
// ──────────────────────────────────────────────────────────────────────────
...
const HandlerDictEntrySchema = z.object({
    ...
    chainCandidates: z.array(z.object({
        caller: z.string(),
        callee: z.string(),
...
export function screenFillPrepDir(projectRoot) {
    return join(specMapDir(projectRoot), SCREEN_FILL_PREP_DIR);
}
/** `.spec/map/screens-fill-frag/` 디렉터리 경로. */
export function screenFillFragDir(projectRoot) {
...
export async function readScreenFillChunkIndex(projectRoot) {
    let raw;
    try {
        raw = await readFile(join(screenFillPrepDir(projectRoot), SCREEN_FILL_PREP_INDEX_FILENAME), 'utf8');
    }
    catch {
...
async function loadLinesCached(projectRoot, relPath, cache) {
...
function readGraphJsps(projectRoot) {
 ...
 */
function resolveViewPath(projectRoot, value, graphJsps) {
 ...
 */
async function expandWithSuperclasses(projectRoot, relPaths, cache) {
 ...
 */
async function scanViewConstants(projectRoot, relPaths, cache, graphJsps) {
    const out = [];
    ...
    for (const relPath of scanPaths.sort(cmp)) {
        ...
        if (out.length >= VIEW_CONSTANT_CAP)
            break;
    }
    return out;
}
 ...
 */
async function buildHandlerDict(projectRoot, targets, fileCache) {
        ...
        for (let depth = 0; depth < CHAIN_DEPTH && chainCandidates.length < CHAIN_CANDIDATE_CAP; depth++) {
            ...
            for (const caller of frontier) {
                ...
                for (const c of calls) {
                    if (!c.calleeFile)
                        continue; // 프로젝트 내부 호출만(external 제외)
                    ...
                    if (chainCandidates.length >= CHAIN_CANDIDATE_CAP)
                        break;
 ...
 */
export async function prepScreenFill(projectRoot, options = {}) {
    ...
    for (let gi = 0; gi < groups.length; gi++) {
        ...
        for (const a of anchors) {
            ...
            if (cur === undefined || a.line < cur)
                anchorByRel.set(a.filePath, a.line);
        }
    ...
    await writeFile(join(prep, SCREEN_FILL_PREP_INDEX_FILENAME), stableJson(ScreenFillChunkIndexSchema.parse(index)), 'utf8');
    return { index, paths };
}
/**
 ...
 */
export async function auditScreenFillFragments(projectRoot, only) {
    ...
    for (const entry of index.chunks) {
        ...
        const chunk = ScreenFillChunkSchema.parse(JSON.parse(await readFile(chunkPath(projectRoot, entry.chunkId), 'utf8')));
...
function recomputeUnmatched(projectRoot, screens, fragments) {
 ...
 */
async function verifyFragmentHandler(projectRoot, bodyHandler, fragHandler, cache) {
    ...
    for (const e of fragHandler.evidence) {
        ...
        const status = await verifyCitation(projectRoot, { filePath: e.file, line: e.line, snippet: e.snippet ?? '' }, cache);
        if (status === 'ok')
            kept.push(e);
        else
            removed++;
...
export async function mergeScreenFillFragments(projectRoot) {
    ...
    const fillByScreen = new Map();
    const declaredScreenIds = new Set();
    const declaredAnnByScreen = new Map();
    let droppedItems = 0;
    const missingScreens = [];
    for (const entry of index.chunks) {
        for (const sid of entry.screenIds)
            declaredScreenIds.add(sid);
        if (!completeSet.has(entry.chunkId)) {
            for (const sid of entry.screenIds)
                missingScreens.push(sid);
            continue;
        }
        const chunk = ScreenFillChunkSchema.parse(JSON.parse(await readFile(chunkPath(projectRoot, entry.chunkId), 'utf8')));
        for (const cs of chunk.screens) {
    ...
    for (const s of file.screens) {
        ...
        screensFilled++;
        const declaredAnn = declaredAnnByScreen.get(s.id) ?? new Set();
