...
export function initialDocState(docId) {
 ...
 */
export function submitForReview(state, actor) {
    if (state.status !== 'DRAFT' && state.status !== 'RETURNED') {
        throw new Error(`[doc-state] submitForReview: illegal transition from ${state.status} (expected DRAFT|RETURNED)`);
    }
    const event = { event: 'SUBMITTED', by: actor.by, at: actor.at };
    return {
        state: {
            ...state,
            status: 'UNDER_REVIEW',
 ...
 */
export function approve(state, doc, actor) {
    if (state.status !== 'UNDER_REVIEW') {
        throw new Error(`[doc-state] approve: illegal transition from ${state.status} (expected UNDER_REVIEW)`);
    }
    const verdict = enforceEvidence(doc);
    if (verdict.ok) {
        const event = { event: 'APPROVED', by: actor.by, at: actor.at };
        return {
            state: {
                ...state,
                status: 'APPROVED',
...
export function returnForRevision(state, actor) {
    if (state.status !== 'UNDER_REVIEW') {
        throw new Error(`[doc-state] returnForRevision: illegal transition from ${state.status} (expected UNDER_REVIEW)`);
    }
    const event = {
        event: 'RETURNED',
        by: actor.by,
