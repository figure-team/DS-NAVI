 ...
 */
export function applyDeterministicLabels(nodes, edges) {
    const flowCountByDomainId = new Map();
    for (const e of edges) {
        if (e.type === 'contains_flow') {
            flowCountByDomainId.set(e.source, (flowCountByDomainId.get(e.source) ?? 0) + 1);
        }
    }
    const nodeCountByKey = new Map();
    for (const n of nodes) {
        const key = n.tags[0];
        if (key)
            nodeCountByKey.set(key, (nodeCountByKey.get(key) ?? 0) + 1);
    }
    const str = (v) => (typeof v === 'string' && v.length > 0 ? v : null);
    return nodes.map((node) => {
        if (node.name !== SKELETON_BLANK)
            return node;
        if (node.type === 'domain') {
 ...
 */
export function emitDomainGraph(projectRoot, skeleton, options = {}) {
 ...
 */
export function demoteUnverified(nodes, report) {
    const verdictByRef = new Map();
    for (const d of report.domains) {
        for (const item of d.items)
            verdictByRef.set(item.ref, item.verdict);
    }
    const mark = (text, ref) => verdictByRef.get(ref) === 'NEEDS_REVIEW' && !text.startsWith(NEEDS_REVIEW_MARKER)
        ...
        if (node.type === 'domain') {
            ...
            for (const [kind, field] of [
                ...
                if (Array.isArray(arr)) {
                    meta[field] = arr.map((text, i) => typeof text === 'string' ? mark(text, `${node.id}#${kind}[${i}]`) : text);
                }
            }
            out.domainMeta = meta;
        }
 ...
 */
export function embedVerification(nodes, report) {
    ...
    return nodes.map((node) => {
        if (node.type === 'domain') {
            ...
            const claims = dr.items.filter((it) => it.kind !== 'flow' && it.kind !== 'step' && it.kind !== 'businessFlow');
            ...
            const bfs = node.domainMeta?.businessFlows;
            const decoratedBfs = Array.isArray(bfs)
                ...
                : undefined;
            if (claims.length === 0 && !decoratedBfs)
                return node;
 ...
 */
export function emitFilledDomainGraph(projectRoot, skeleton, filledNodes, options = {}) {
