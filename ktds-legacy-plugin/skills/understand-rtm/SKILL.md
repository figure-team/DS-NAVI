...
2. **실행**
   ...
   - 이 실행은 루트 슬롯(`.spec/map/impact.json`)·문서 09·구조 오버레이를 **건드리지 않는다**.
     결과는 `<세션>/impact-run.json` 포인터 + `<세션>/impact/` 스냅샷으로 **세션 디렉터리에만**
     보관된다(연합 — impact-history 원장 직접 기록 없음, IMPACT_LEDGER_FEDERATION_DESIGN §2.1).
     `/change` 메뉴 열람은 대시보드 서버가 세션 포인터를 읽기 시점에 병합해 제공한다.
