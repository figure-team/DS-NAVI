 ...
 */
export function buildWikiVault(docs, resolveMeta) {
    const sorted = sortDocs(docs);
    const byKey = new Map();
    for (const doc of sorted)
        byKey.set(semanticKey(doc.docId), doc);
    ...
    for (const doc of sorted) {
        ...
        const content = related.length === 0
            ? body
            : `${body.replace(/\n+$/, '\n')}${related.join('\n')}\n`;
