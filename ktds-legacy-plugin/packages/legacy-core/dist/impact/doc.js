...
export const IMPACT_READONLY_NOTE = '이 문서는 **읽기전용 분석 산출물**입니다 — 검토·승인 상태기계(DRAFT→APPROVED) 밖이며 ' +
    '`[생성]` 예측은 net-new 라 CONFIRMED 를 받지 못합니다(선례 앵커만 실존 근거).';
function ev(file, line) {
    return { file, line: line ?? null };
}
function tally(upPaths, downPaths, labelOf) {
    const rows = new Map();
    const bump = (relPath, dir) => {
...
function aggregateTable(heading, rows) {
...
function buildAggregateSection(result, inputs) {
...
export function buildChangeImpact(result, verify, options = {}) {
    const verdict = new Map();
    for (const it of verify.items)
        verdict.set(`${it.kind}|${it.ref}`, it.verdict);
    const confFor = (kind, ref) => {
    ...
    const seedClaims = result.seeds.map((s) => claim(`변경 시드: ${s.relPath} (origin: ${s.origin})`, s.confidence, [ev(s.relPath, null)]));
    ...
    const flowClaims = result.upstream.flows.map((f) => claim(`흐름 영향: ${f.flowId} → 도메인 ${f.domainName ?? f.domainKey ?? '(미상)'} (검출 ${f.via})`, f.confidence));
    const domainClaims = result.upstream.domains.map((d) => claim(`도메인 영향: ${d.name ?? d.key}`, d.confidence));
    const mapperClaims = result.upstream.persistence.mappers.map((m) => {
        ...
        return claim(`영속성 영향(매퍼): ${m.relPath}${ns}${owners}`, confFor('mapper', m.relPath), m.citation ? [ev(m.citation.filePath, m.citation.line)] : []);
    });
    const sqlClaims = result.upstream.persistence.sqlFiles.map((s) => claim(`영속성 영향(SQL): ${s.relPath}`, 'INFERRED', [ev(s.relPath, null)]));
    ...
    const jpaClaims = result.upstream.persistence.jpaTables.map((t) => {
        ...
        return claim(`영속성 영향(JPA): ${t.entityClass} → 테이블 ${t.tableName}${naming}${cols}`, t.confidence, [ev(t.citation.filePath, t.citation.line)]);
    });
    const dbProse = [
    ...
    const upstreamClaims = result.upstream.files.map((f) => claim(`연관 모듈(상류): ${f.relPath} (via ${f.viaKinds.join(',')}, 깊이 ${f.minDepth})`, f.citation ? confFor('upstream', f.relPath) : 'INFERRED', f.citation ? [ev(f.citation.filePath, f.citation.line)] : []));
    const downstreamClaims = result.downstream.files.map((f) => claim(`연관 협력(하류): ${f.relPath} (via ${f.viaKinds.join(',')}, 깊이 ${f.minDepth})`, f.citation ? confFor('downstream', f.relPath) : 'INFERRED', f.citation ? [ev(f.citation.filePath, f.citation.line)] : []));
...
export function publishChangeImpact(projectRoot, doc, meta) {
    const docMeta = {
        docId: doc.docId,
        title: doc.title,
 ...
 */
export function toProfileWChangeStory(suggestion, result) {
    ...
    for (const c of suggestion.create) {
        ...
        if (c.suggestedPath)
            fileList.add(c.suggestedPath);
        for (const a of [...c.precedentAnchors, ...c.conventionAnchors])
            citations.push({ file: a.file, line: a.line });
    }
    const dedupCitations = [...new Map(citations.map((c) => [`${c.file}:${c.line}`, c])).values()].sort((a, b) => cmp(a.file, b.file) || (a.line ?? 0) - (b.line ?? 0));
