...
function toIface(item: InterfaceReport['items'][number]): z.infer<typeof SystemMapInterfaceSchema> {
  return {
    id: item.id,
...
export function buildSystemMap(input: {
  ...
  const embedded =
    dbSchema.liveDbSignals.length > 0 && dbSchema.liveDbSignals.every((s) => s.embedded)
 ...
 */
export function writeSystemMap(projectRoot: string, systemMap: SystemMap): string {
