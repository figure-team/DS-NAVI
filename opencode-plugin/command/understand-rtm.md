...
```
node $ATLAS_PLUGIN_ROOT/scripts/understand-rtm.mjs <projectRoot>
```
...
- 템플릿 로드: 프로젝트 override `.understand-anything/templates/requirements/0X_*.md` → 없으면 플러그인
  동봉 `$ATLAS_PLUGIN_ROOT/templates/requirements/0X_*.md`. **빈 템플릿 구조만 보고 채운다(examples 미참조).**
...
0. **요청ID 부여**: `node $ATLAS_PLUGIN_ROOT/scripts/rtm-intake.mjs next-req <projectRoot>` 로 충돌 없는 다음
...
6. **검증**: `node $ATLAS_PLUGIN_ROOT/scripts/rtm-intake.mjs validate <세션>/identified.json` 실행.
...
**두 명령을 순서대로 실행한다:**
```
node $ATLAS_PLUGIN_ROOT/scripts/rtm-intake.mjs project <projectRoot> <sid>
node $ATLAS_PLUGIN_ROOT/scripts/understand-rtm.mjs <projectRoot>
```
...
### 1) CR 번호 부여(결정론)
```
node $ATLAS_PLUGIN_ROOT/scripts/rtm-intake.mjs next-cr <projectRoot>
```
...
### 2) 영향 분석(RTM 역추적)
```
node $ATLAS_PLUGIN_ROOT/scripts/rtm-intake.mjs impact <projectRoot> <REQ> --out <projectRoot>/.understand-anything/change/<CR>/impact.json
```
...
`$ATLAS_PLUGIN_ROOT/templates/requirements/0X_*.md`. **빈 칸 금지·추적 칸 비우지 않기.** 산출 위치는
...
### 4) 폐기 표시(원장 반영) + 재bake
```
node $ATLAS_PLUGIN_ROOT/scripts/rtm-intake.mjs withdraw <projectRoot> <REQ> <CR> "<사유>"
node $ATLAS_PLUGIN_ROOT/scripts/understand-rtm.mjs <projectRoot>
```
