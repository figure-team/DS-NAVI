 ...
 */
export function normalizedBusinessFlows(fill) {
 ...
 * 의도적으로 허용한다.
 */
export function validateBusinessFlow(bf, domainFlowIds) {
    const errors = [];
    const ids = new Set();
    ...
    for (const e of bf.edges) {
        if (!ids.has(e.from))
            errors.push(`edge-from-unknown: ${e.from}`);
        if (!ids.has(e.to))
            errors.push(`edge-to-unknown: ${e.to}`);
        touched.add(e.from);
        touched.add(e.to);
    ...
    if (starts < 1)
        errors.push('no-start-node');
    if (ends < 1)
        errors.push('no-end-node');
    for (const n of bf.nodes) {
        ...
        if (n.kind === 'decision') {
            ...
            if (outs.length < 2)
                errors.push(`decision-needs-branches: ${n.id} (outgoing ${outs.length})`);
            if (outs.some((e) => !e.label))
                errors.push(`decision-branch-unlabeled: ${n.id}`);
        }
    }
    return errors.sort(cmp);
}
/** `.spec/map/fill/` 디렉터리 경로. */
export function fillDir(projectRoot) {
    return join(specMapDir(projectRoot), FILL_DIR);
}
/** 도메인 key 에 대응하는 fill 파일의 절대 경로. */
export function fillPathFor(projectRoot, key) {
 ...
 */
export async function readFills(projectRoot, skeleton) {
    const fills = [];
    const pending = [];
    const invalid = [];
...
export function applyFills(skeleton, fills) {
    ...
    for (const fill of [...fills].sort((a, b) => cmp(a.domainId, b.domainId))) {
        ...
        if (bfList.length > 0) {
            const domainFlowIds = new Set(nodes.filter((n) => n.type === 'flow' && domainKeyOf(n) === key).map((n) => n.id));
            const survivors = [];
            const reasons = [];
            bfList.forEach((bf, i) => {
...
export function unfilledNodes(nodes) {
    return nodes
