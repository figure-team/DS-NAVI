...
import { DEFAULT_STEP_CAP, SKELETON_BLANK, } from './types.js';
 ...
 */
function domainComplexity(fileCount) {
    if (fileCount < 8)
        return 'simple';
    if (fileCount < 20)
        return 'moderate';
 ...
 *   complex  : >= 7 step
 */
function flowComplexity(stepCount) {
    if (stepCount <= 3)
        return 'simple';
    if (stepCount <= 6)
        return 'moderate';
 ...
 */
export async function buildSkeleton(projectRoot, input, options = {}) {
    const { census, routes, edges, slices, candidates, plan, methodCallGraph } = input;
    if (!plan) {
        throw new Error('skeleton requires a ConfirmedPlan — run /understand-map confirm first');
    }
    ...
    for (const d of domains) {
        ...
        for (const alias of d.aliasKeys)
            domainByKey.set(alias, d.key);
        for (const root of d.roots)
            domainByRoot.set(root, d.key);
    }
    ...
    for (const cand of candidates.candidates) {
        ...
        for (const f of cand.files) {
            if (!domainByFile.has(f.relPath))
                domainByFile.set(f.relPath, key);
        }
    }
    for (const [root, key] of domainByRoot)
        domainByFile.set(root, key);
    const fileCountByDomain = new Map();
    ...
    for (const r of routes.routes) {
        ...
        if (list)
            ...
            routesByFile.set(r.filePath, [r]);
    }
    const batchByFile = new Map();
    for (const b of routes.batchEntries) {
        const list = batchByFile.get(b.filePath);
        if (list)
            list.push(b);
        else
            batchByFile.set(b.filePath, [b]);
    }
    ...
    const methodsByFilePair = new Map();
    for (const c of methodCallGraph?.calls ?? []) {
        ...
        if (!list)
            methodsByFilePair.set(key, (list = []));
        if (!list.includes(c.calleeMethod))
            list.push(c.calleeMethod);
    }
    const layerSignals = buildLayerSignals(routes, edges);
    ...
    for (const d of domains) {
        ...
        for (const root of [...d.roots].sort(cmp)) {
            ...
            for (const flow of flows) {
                ...
                let ordered;
                const traced = methodCallGraph && flow.handlerMethod
                ...
                if (traced && traced.length > 1) {
                    ...
                    // 나머지는 relPath 정렬.
                    const reached = reachedByRoot.get(root) ?? new Set([root]);
                ...
                for (const fileA of stepFiles) {
                    ...
                    for (const fileB of [...targets].sort(cmp)) {
                        if (fileB === fileA || !inChain.has(fileB))
                            continue;
    ...
    const nodeTypeOrder = { domain: 0, flow: 1, step: 2 };
    ...
    edgeList.sort((a, b) => edgeTypeOrder[a.type] - edgeTypeOrder[b.type] ||
 ...
 */
function handlerDeclaredIn(javaFacts, root, method) {
 ...
 */
function collectFlows(root, routesByFile, batchByFile) {
    const byId = new Map();
    const consider = (f) => {
        const prev = byId.get(f.flowId);
        if (!prev || f.line < prev.line)
            byId.set(f.flowId, f);
    };
...
async function loadJavaFacts(projectRoot, census) {
    const out = new Map();
