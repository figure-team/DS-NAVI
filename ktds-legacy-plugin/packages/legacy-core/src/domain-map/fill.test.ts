...
// 17.2 채움 계약 — 인용 의무 스키마, 구조 read-only 적용, 항목 단위 기각.
...
function orderFill(): DomainFill {
  return {
    ...
    flows: [
      { flowId: "flow:POST /orders", name: "주문 생성", summary: { text: "신규 주문 접수", citations: [CITE] } },
    ],
    steps: [
...
test("스키마: 사실 주장은 인용 없이 통과 불가 (citations min 1)", () => {
  const bad = orderFill();
...
test("적용: 의미 필드만 채워지고 구조 필드는 불변, 인용은 ktdsClaims 동봉", () => {
  const sk = skeleton();
...
test("기각: 모르는 ID·다른 도메인의 ID는 항목 단위로 보고된다", () => {
  const fill = orderFill();
  fill.flows.push({
    flowId: "flow:GET /ghost",
    name: "유령",
    summary: { text: "x", citations: [CITE] },
  });
  fill.steps.push({
...
test("기각: 다른 도메인 소속 노드를 채우려는 시도", () => {
  const fill = orderFill();
  // member 도메인 노드를 order fill이 건드리려 함
  fill.flows = [];
  fill.steps = [];
  const sk = skeleton();
  // member 도메인에 flow 추가
...
test("unfilledNodes: 빈칸 잔여 노드 식별 (디스패치 진행률)", () => {
  const sk = skeleton();
  const { nodes } = applyFills(sk, [orderFill()]);
