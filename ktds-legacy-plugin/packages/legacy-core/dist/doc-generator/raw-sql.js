 ...
 */
export function extractSqlCrud(source, knownTables) {
    if (knownTables.size === 0)
        return [];
    const firstLine = new Map(); // `${table}|${crud}` → line
 ...
 */
export function buildRawSqlModel(files, knownTables) {
    const byFile = {};
    for (const { relPath, content } of files) {
        const accesses = extractSqlCrud(content, knownTables);
        if (accesses.length > 0)
            byFile[relPath] = accesses;
    }
    return { byFile };
}
/** 모델이 비었나(축을 세울 SQL 신호가 하나도 없음). */
export function isRawSqlModelEmpty(model) {
    return !model || Object.keys(model.byFile).length === 0;
}
//# sourceMappingURL=raw-sql.js.map