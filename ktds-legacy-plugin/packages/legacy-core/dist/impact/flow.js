...
/** flowId → routeId(꼬리표 보존). batch flow 는 라우트가 아니라 null. */
function flowToRoute(flowId) {
...
flowImpactSet, skeleton, ownership, routes, confirmed) {
    ...
    const touchFlow = (flowId, domainId, stepId, via) => {
        ...
        if (domainId && !prev.domainId)
            prev.domainId = domainId;
        if (stepId && (prev.viaStepId === null || stepId < prev.viaStepId))
            prev.viaStepId = stepId;
    };
    ...
    if (skeleton) {
        ...
        for (const s of skeleton.stepSources) {
            ...
            if (list)
                ...
                stepsByFile.set(s.relPath, [s.stepId]);
        }
        const flowByStep = new Map();
        const domainByFlow = new Map();
        for (const e of skeleton.edges) {
            if (e.type === 'flow_step')
                flowByStep.set(e.target, e.source);
        ...
        // cap 절단 결손큐: 잘린 파일이 영향집합에 있으면 그 flow 는 영향 가능성 있으나
        // step 미수록(조용한 누락 금지).
        ...
        needsReview.push({
            ref: 'skeleton',
            reason: '도메인 맵 미확정(confirm 전) — ownership 폴백, 도메인명 [확인 필요]',
        });
    }
    // ownership 폴백: skeleton step 에 안 잡힌 영향 파일은 ownership[f].owners→
    // 그 root 가 선언한 라우트→flowId. 도메인 링크 없음 → UNVERIFIED.
    for (const f of flowImpactSet) {
        if (covered.has(f))
            continue;
    ...
    const domains = [...domainAcc.entries()]
        .map(([domainId, d]) => ({
        domainId,
        key: d.key,
        name: d.name,
        confidence: (d.name ? 'INFERRED' : 'UNVERIFIED'),
    }))
