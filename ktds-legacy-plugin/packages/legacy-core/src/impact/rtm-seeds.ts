...
const confRank = (c: string): number => {
  const i = (CONFIDENCE_VALUES as readonly string[]).indexOf(c)
...
export interface FlowSeedResolution {
  ...
  bySource: Array<{ fnId: string; relPaths: string[] }>
 ...
 */
export function resolveFlowSeeds(
  functions: readonly RtmFunctionRow[],
  fnIds: readonly string[],
): FlowSeedResolution {
  const byId = new Map(functions.map((f) => [f.id, f]))
  /** relPath → 시드. 여러 flow 가 같은 파일을 가리키면 **가장 강한 신뢰도**를 남긴다(강등 금지). */
  const seedByPath = new Map<string, ImpactSeed>()
  const bySource: FlowSeedResolution['bySource'] = []
  const skippedToBe: string[] = []
  const unknownFnIds: string[] = []
  const ungroundedFnIds: string[] = []
