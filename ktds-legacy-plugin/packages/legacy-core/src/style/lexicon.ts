 ...
 */
export function parseLexicon(md: string): LexiconEntry[] {
  const lines = md.split(/\r?\n/)
  const isRow = (l: string) => l.trim().startsWith('|')
  const isSeparator = (l: string) => /^\|?\s*:?-{2,}/.test(l.trim().replace(/^\|/, '').trim())
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // 헤더 행: 바로 다음 줄이 구분선.
    if (i + 1 < lines.length && isRow(lines[i + 1]) && isSeparator(lines[i + 1])) continue
    const cells = line
      .trim()
      .replace(/^\|/, '')
      .replace(/\|$/, '')
...
export function applyLexiconToText(
  text: string,
 ...
 */
export function applyLexiconDeep<T>(value: T, entries: LexiconEntry[]): { value: T; hits: number } {
  if (entries.length === 0) return { value, hits: 0 }
  let hits = 0
  const walk = (node: unknown, keyInParent: string | null): unknown => {
    if (node === null || node === undefined) return node
    if (Array.isArray(node)) return node.map((item) => walk(item, keyInParent))
  ...
  return { value: walk(value, null) as T, hits }
}
