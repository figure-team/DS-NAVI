...
function parseProperties(text, relPath, out) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const key = m[1];
        if (!out.has(key))
            out.set(key, { value: m[2], file: relPath, line: i + 1 });
    }
}
/** yaml 1파일 단순 flatten — 스칼라 매핑만. `a:\n  b: v` → `a.b=v`. */
function parseYamlFlat(text, relPath, out) {
    const lines = text.split('\n');
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!out.has(path))
            out.set(path, { value: unquoted, file: relPath, line: i + 1 });
    }
}
/** census 에서 설정 파일(lang=properties|yaml)을 골라 프로퍼티 인덱스를 만든다. */
export function buildPropertyIndex(projectRoot, census) {
    const out = new Map();
    ...
    for (const f of files) {
        ...
        if (f.lang === 'properties')
            parseProperties(text, f.relPath, out);
        else
            parseYamlFlat(text, f.relPath, out);
    }
    return out;
}
 ...
 */
export function resolvePlaceholders(raw, props) {
    if (!raw.includes('${'))
        return { resolved: raw, resolvedFrom: null };
    let resolvedFrom = null;
    let failed = false;
    const resolved = raw.replace(PLACEHOLDER_RE, (_all, key, def) => {
        const entry = props.get(key.trim());
        if (entry) {
            if (resolvedFrom === null)
                resolvedFrom = `${entry.file}:${entry.line}`;
            return entry.value;
        }
        if (def !== undefined)
            return def;
