...
export async function runFillPipeline(projectRoot, options = {}) {
    ...
    // 건너뛴다(부분 수용).
    const rejectedBusinessFlows = new Set(rejected.filter((r) => r.kind === 'businessFlow').map((r) => r.ref));
    ...
    const staleSkeleton = skeleton.gitCommit !== null && currentCommit !== null && skeleton.gitCommit !== currentCommit;
    return {
        pending,
        invalid,
        rejected,
