...
/** 확장자(소문자, 점 제외) -> 언어. */
export const SOURCE_LANG_BY_EXT = {
    java: 'java',
    xml: 'xml',
    jsp: 'jsp',
    ts: 'typescript',
 ...
 */
export function isSkippedSegment(seg) {
    if (WALK_SKIP_DIRS.has(seg))
        return true;
    return WALK_SKIP_DIR_BASES.some((base) => seg === base || (seg.startsWith(base) && SKIP_BASE_SUFFIX_SEP.test(seg.slice(base.length))));
}
...
function listGitFiles(projectRoot) {
    try {
        const out = execFileSync('git', ['-C', projectRoot, 'ls-files', '--cached', '--others', '--exclude-standard'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'], maxBuffer: 64 * 1024 * 1024 });
        return out
            .split('\n')
            .map((l) => l.trim())
            .filter((l) => l.length > 0)
...
function applyGitignore(projectRoot, files) {
...
/** 프로젝트 파일 인구조사를 만든다. */
export function buildCensus(projectRoot) {
