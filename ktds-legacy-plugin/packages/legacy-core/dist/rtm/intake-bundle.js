 ...
 *   투성이 보일러플레이트라 최하위.
 */
export const POLICY_SECTION_PRIORITY = [
    { rank: 0, pattern: /미결/ },
...
function strList(v) {
    return arr(v).filter((x) => typeof x === 'string');
}
 ...
 */
function parsePolicyEvidence(cell) {
    const out = [];
    for (const m of cell.matchAll(/`([^`]+)`/g)) {
        const ref = m[1].trim();
        const lm = /^(.*?):(\d+)(?:[-,]\d+)*$/.exec(ref);
        if (lm)
            out.push({ file: lm[1], line: parseInt(lm[2], 10), snippet: null });
        else if (ref.length > 0 && !ref.includes(' '))
            out.push({ file: ref, line: null, snippet: null });
    }
    return out;
}
// ── 사전 필터(결정론, LLM 없음) ──────────────────────────────────────────────
...
function matchTokens(tokens, haystack) {
 ...
 * 부당하게 유리해진다).
 */
function matchCoverage(matched, name) {
    if (name.length === 0)
        return 0;
...
export function checkMinimalSet(sources) {
    const missing = [];
    if (sources.domainGraph === null)
        missing.push('도메인: .understand-anything/domain-graph.json (생성: /understand-map)');
    if (sources.dbSchema === null)
        missing.push('데이터: .spec/map/db-schema.json (생성: /understand-map)');
    if (sources.rtm === null)
        missing.push('추적표: .understand-anything/rtm.json (생성: /understand-rtm)');
    return { ok: missing.length === 0, missing };
}
/** tags[0] 로 도메인 귀속을 잡는다 — 실측상 domain/flow/step 전 노드가 단일 태그를 갖는다. */
function groupDomainScopes(graph) {
    // 원시 JSON 이라 null·숫자 등 쓰레기 원소가 섞일 수 있다 — 죽지 말고 건너뛴다.
    const nodes = arr(obj(graph).nodes).filter((n) => n !== null && typeof n === 'object' && !Array.isArray(n));
    const byKey = new Map();
...
function buildDomainAxis(graph, tokens) {
    const empty = {
        present: false,
    ...
    if (graph === null)
        return { axis: empty, selectedKeys: new Set(), fallback: null };
    const g = obj(graph);
    ...
    let cited = 0;
    let claimsTotal = 0;
    const items = picked.map(({ s, matched }) => {
        const meta = obj(s.node.domainMeta);
        const claims = arr(meta.ktdsClaims);
        claimsTotal += claims.length;
        for (const c of claims) {
            if (arr(obj(c).citations).length > 0)
                cited += 1;
        }
        ...
        const files = [...new Set([...s.steps, ...s.flows].map((n) => str(n.filePath)).filter((f) => f !== null))].sort(natCmp);
        return {
...
function buildCrudAxis(matrix, tokens) {
    const empty = {
        present: false,
    ...
    if (matrix === null)
        return { axis: empty, touchedTables: new Map(), fallback: null };
    const m = obj(matrix);
    ...
    if (picked.length === 0) {
        ...
        picked = rows
            ...
            const o = obj(r);
            const cells = strList(o.cells);
            return { o, cells, feature: cells[0] ?? '', matched: [] };
        })
    ...
    picked.sort((a, b) => b.matched.length - a.matched.length ||
    ...
    const touchedTables = new Map();
    let cited = 0;
    const items = capped.map(({ o, cells, feature, matched }, rowRank) => {
        const nonEmpty = [];
        for (let i = 1; i < columns.length; i++) {
            const ops = cells[i];
            if (ops && ops.length > 0) {
                nonEmpty.push({ table: columns[i], ops });
                if (!touchedTables.has(columns[i]))
                    touchedTables.set(columns[i], rowRank);
            }
        }
 ...
 */
function countMentions(name, haystack) {
    const esc = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    return (haystack.match(new RegExp(`\\b${esc}\\b`, 'gi')) ?? []).length;
}
function buildSchemaAxis(dbSchema, tokens, touchedTables, 
...
mentionText) {
    ...
    if (dbSchema === null)
        return { axis: empty, fallback: null };
    ...
    let picked = tables
        ...
        const selectedBy = [];
        if (byToken)
            selectedBy.push('token');
        if (crudRank !== undefined)
            selectedBy.push('crud');
    ...
    if (picked.length === 0) {
        ...
        picked = tables
            ...
            const o = obj(t);
            const name = str(o.name) ?? '';
            return { o, name, selectedBy: [], crudRank: Number.MAX_SAFE_INTEGER, mentions: countMentions(name, mentionText) };
        })
    ...
    picked.sort((a, b) => Number(b.selectedBy.includes('token')) - Number(a.selectedBy.includes('token')) ||
    ...
    let cited = 0;
    let colTotal = 0;
    const items = capped.map(({ o, name, selectedBy }) => {
...
function compactMechanical(v) {
    ...
    for (const k of Object.keys(o).sort(natCmp)) {
        ...
        if (val === null || val === undefined)
            continue;
        if (val === false)
            continue; // required:false 등 — 기본값이라 정보가 없다.
...
function projectAnnotation(a) {
    ...
    return {
        ...
        handler: Object.keys(h).length > 0
...
function annotationRank(a, tokens, ownAction) {
    ...
    if (a.mechanical !== null && a.mechanical.inputType !== undefined)
        return { rank: 2, matched };
...
function buildScreensAxis(screens, tokens, selectedDomainKeys) {
    ...
    if (picked.length === 0) {
        ...
        picked = all
            .map((sc) => {
            const o = obj(sc);
            return { o, id: str(o.id) ?? '', matched: [], selectedBy: [], title: str(o.title) ?? '' };
        })
...
const isTableSeparator = (cells) => cells.length > 0 && cells.every((c) => /^:?-{3,}:?$/.test(c));
...
function policyDomainKey(relPath) {
 ...
 */
function buildPolicyAxis(docs, tokens, selectedDomainKeys) {
    const empty = {
        present: false,
    ...
    if (picked.length === 0) {
        ...
        picked = parsed.slice(0, FALLBACK_TOP_N.policyDocs).map((x) => ({ ...x, selectedBy: [] }));
    }
    // 정렬: 도메인 조인 > 토큰 > shared. 트림은 역순이라 shared(행 0건 통계)가 먼저 빠진다.
    const docRank = (sel) => sel.includes('domain') ? 0 : sel.includes('token') ? 1 : 2;
    ...
    const stamps = [...new Set(items.map((i) => i.sourceCommit).filter((c) => c !== null))];
    return {
        axis: {
            present: true,
            source: '.understand-anything/doc-output/policy-*.md',
...
function buildRtmAxis(rtm, tokens, selectedDomainKeys) {
    const empty = {
        present: false,
    ...
    let picked = fns
        ...
        if (matched.length === 0 && !byDomain)
            return null;
        return { o, id, name, domainId, matched, selectedBy: (matched.length > 0 ? 'token' : 'domain') };
    })
    ...
    if (picked.length === 0) {
        ...
        picked = fns
            .map((f) => {
            const o = obj(f);
            return { o, id: str(o.id) ?? '', name: str(o.name) ?? '', domainId: str(o.domainId), matched: [], selectedBy: 'domain' };
        })
            .filter((x) => x.id.length > 0)
            .sort((a, b) => natCmp(a.id, b.id))
    ...
    picked.sort((a, b) => (a.selectedBy === b.selectedBy ? 0 : a.selectedBy === 'token' ? -1 : 1) ||
    ...
    const items = capped.map(({ o, id, name, domainId, selectedBy }) => ({
        id,
        name,
        domainId,
 ...
 */
export function serializeIntakeBundle(bundle) {
 ...
 */
function axisSize(items, extraDepth) {
 ...
 */
export function allocateAxisBudget(demand, available) {
    const keys = Object.keys(AXIS_BUDGET);
    const alloc = {};
    ...
    if (floorSum > available && floorSum > 0) {
        ...
        for (const k of keys)
            alloc[k] = Math.floor((Math.min(demand[k], AXIS_BUDGET[k].floor) * available) / floorSum);
        return alloc;
    }
    for (const k of keys)
        alloc[k] = Math.min(demand[k], AXIS_BUDGET[k].floor);
    let left = available - floorSum;
    for (;;) {
        const hungry = keys.filter((k) => demand[k] > alloc[k]);
        if (hungry.length === 0 || left <= 0)
            break;
 ...
 */
function trimAxisTo(items, limit, depth, omitted, label, 
 ...
 */
function trimToCharCap(bundle, charCap) {
    ...
    trimmers.push({
        ...
        drop: () => {
            const i = bundle.axes.policy.items.map((d) => d.emptyArtifact).lastIndexOf(false);
            if (i >= 0)
                bundle.axes.policy.items.splice(i, 1);
        },
    });
    ...
    trimmers.push({
        ...
        drop: () => {
            const i = bundle.axes.rtm.items.map((f) => f.selectedBy).lastIndexOf('domain');
            if (i >= 0)
                bundle.axes.rtm.items.splice(i, 1);
        },
    });
    trimmers.push({
        label: () => {
 ...
 */
function trimPolicyAxis(bundle, limit) {
    ...
    while (over()) {
        ...
        if (i < 0)
            break; // 남은 게 전부 §4.1 스텁 — 더 버리지 않는다(값싸고 오독 차단에 필수).
 ...
 */
function applyAxisBudget(bundle, charCap) {
    const A = bundle.axes;
    const demand = {
    ...
    [A.domain.items, A.data.schema.items, A.data.crud.items, A.rtm.items, A.screens.items, A.policy.items] = saved;
    ...
    for (const k of Object.keys(AXIS_BUDGET)) {
...
function resyncCounts(bundle) {
    const sync = (axis) => {
        const dropped = axis.selected - axis.items.length;
        if (dropped > 0)
            axis.omittedCount += dropped;
 ...
 */
export function buildIntakeInputBundle(sources, options) {
    ...
    const domainJoin = domain.fallback === null ? domain.selectedKeys : new Set();
    ...
    if (!crud.axis.present)
        warnings.push('CRUD 매트릭스(.spec/map/crud-matrix.json) 없음 — 기능↔테이블 접근을 "없음"으로 읽지 마십시오. 못 본 것입니다.');
    if (commits.note)
        warnings.push(commits.note);
    ...
    for (const d of policy.axis.items) {
        if (d.emptyArtifact) {
            warnings.push(`${d.relPath}: 데이터 행 **0건**(선언 evidenceRate=${d.declaredEvidenceRate ?? 'null'}) — ` +
    ...
    const omittedAxes = [];
    if (!screens.axis.present)
        omittedAxes.push('화면(.understand-anything/screens.json)');
    if (!policy.axis.present)
        omittedAxes.push('정책(.understand-anything/doc-output/policy-*.md)');
    const reducedMode = {
        active: omittedAxes.length > 0,
        omittedAxes,
        note: omittedAxes.length > 0
    ...
    if (reducedMode.note)
        warnings.push(reducedMode.note);
    const fallbacks = [domain.fallback, schema.fallback, crud.fallback, rtm.fallback, screens.fallback, policy.fallback].filter((f) => f !== null);
    const activeAxes = [domain.axis, schema.axis, crud.axis, rtm.axis, screens.axis, policy.axis].filter((a) => a.present).length;
    const mode = fallbacks.length === 0 ? 'token' : fallbacks.length >= activeAxes ? 'fallback' : 'mixed';
    const bundle = {
        schemaVersion: 2,
    ...
    for (let round = 0; round < 4; round++) {
        ...
        if (serializeIntakeBundle(bundle).length <= charCap)
            break;
    }
...
//# sourceMappingURL=intake-bundle.js.map